...
export const agentAPI = {
    ...
    async changeStatus(userId, newStatus) {
        // Pega o status anterior para fazer a conta do tempo
        ...
        if (profile.status && profile.status_updated_at) {
            ...
            // Salva o registro de tempo (Timesheet)
    ...
    async getPendingTickets() {
        const { data, error } = await supabase.from('tickets')
            .select(`
                id, protocol_number, channel, created_at, status, agent_id, 
                last_sender, last_interaction_at, is_upload_enabled, 
    ...
    async closeTicket(ticketId, tag2Text) {
        const { error } = await supabase.from('tickets').update({ 
            status: 'closed', tag2_detail: tag2Text, closed_at: new Date(), is_upload_enabled: false 
        }).eq('id', ticketId);
        if (error) throw error;
    },
    ...
    async releaseMyTickets(agentId) {
        const { error } = await supabase.from('tickets').update({ 
            agent_id: null, status: 'open', is_upload_enabled: false 
    ...
    async getActiveAgents() {
        const { data, error } = await supabase.from('profiles')
            .select('id, full_name, can_web, can_email')
            .eq('is_approved', true)
            .eq('status', 'online') // Agora só busca quem estiver oficialmente Online
            .order('full_name');
        if (error) throw error;
        return data;
    },

    // 3. Mensagens e Arquivos
    ...
    // 5. Equipe e Relatórios de Tempo
    async getTeamProfiles() {
        const { data, error } = await supabase.from('profiles')
            .select('*, agent_skills(subject_id)')
            .order('created_at', { ascending: true });
        if (error) throw error;
        return data;
    },
    ...
    async getAgentLogsToday() {
        ...
        const { data, error } = await supabase.from('agent_status_logs')
            .select('*')
    ...
    async getAllSubjects() {
        const { data, error } = await supabase.from('ticket_subjects')
