...
export const agentAPI = {
    ...
    async closeTicket(ticketId, tag2Text) {
        const { error } = await supabase.from('tickets').update({ status: 'closed', tag2_detail: tag2Text, closed_at: new Date(), is_upload_enabled: false }).eq('id', ticketId);
        if (error) throw error;
    },
    ...
    async getActiveAgents() {
        const { data, error } = await supabase.from('profiles').select('id, full_name, can_web, can_email').eq('is_approved', true).eq('status', 'online').order('full_name');
        if (error) throw error;
        return data;
    },
