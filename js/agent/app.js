...
const App = {
    ...
    async init() {
        ...
        try {
            ...
            console.warn("Aviso: Configurações no Banco ausentes. Usando locais.", e);
            this.systemSettings = { 
                chat_timeout_min: 10, 
                email_timeout_hr: 24, 
                closure_macro: "Olá, [nome do cliente]!\n\nFicamos aguardando o seu retorno, mas não tivemos sucesso! Estamos encerrando o seu atendimento por aqui. Se ainda precisar de ajuda sobre esse assunto, basta nos chamar novamente e informar o número do protocolo [protocolo].\n\nDessa forma, seu atendimento será agilizado e conseguiremos te ajudar rapidamente! Até logo!" 
            };
        }
    ...
    async executeAutoClose(ticket) {
        try {
            ...
            console.error("Falha ao rodar macro de encerramento:", e);
        } finally {
    ...
    async loadSettingsUI() {
        try {
            ...
            console.error("Erro ao carregar configurações de UI", e);
        }
    },
    ...
    async pickTicket(id) {
        try {
            ...
            if (t.channel === 'email') {
                ...
                document.getElementById('chat-header-channel').innerHTML = `<span class="material-symbols-outlined text-[14px]">mail</span> E-MAIL`;
                ...
                document.getElementById('chat-header-name').innerText = t.customers?.full_name || 'Desconhecido'; 
                document.getElementById('chat-header-protocol').innerText = `HZ-${t.protocol_number}`;
                document.getElementById('chat-header-channel').innerHTML = `<span class="material-symbols-outlined text-[14px]">forum</span> CHAT WEB`;
                document.getElementById('chat-header-subject').innerText = `• ${t.ticket_subjects?.label || 'Sem assunto'}`;
            ...
            console.error("Erro Crítico ao abrir ticket:", e);
