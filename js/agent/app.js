...
const App = {
    ...
    init() {
        ...
        document.getElementById('login-form').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                this.applyWatermark(profile.full_name); // Aplica a marca d'água de segurança
        ...
        // Envio de Arquivo/Anexo pelo Agente
    ...
    applyWatermark(name) {
        ...
        let spans = '';
        for(let i=0; i<150; i++) {
            spans += `<span class="transform -rotate-45 text-2xl font-black whitespace-nowrap p-6 text-slate-900">${name}</span>`;
        }
        wm.innerHTML = spans;
    ...
    renderBubbles() {
        ...
        // Ordena para que os tickets atualizados por último fiquem mais à direita
    ...
    async loadTeam() {
        ...
        tbody.innerHTML = team.map(member => {
            ...
            <tr class="relative z-20">
                <td class="p-5 font-black text-slate-900">${member.full_name}</td>
                <td class="p-5 text-xs font-bold uppercase text-slate-500">${member.role}</td>
