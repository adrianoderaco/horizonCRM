...
const App = {
    ...
    async init() {
        ...
        try {
            ...
            console.warn("Usando configs locais", e);
            this.systemSettings = { chat_timeout_min: 10, email_timeout_hr: 24, closure_macro: "Encerrado por inatividade." };
        }
        ...
        document.getElementById('login-form').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (profile.role === 'gestor') { 
                    ...
                    document.getElementById('btn-settings').classList.remove('hidden-view'); // MOSTRA A ENGRENAGEM
                    this.renderDashboard(); 
                }
                
                const toggleRouteBtn = document.getElementById('toggle-routing');
                if (toggleRouteBtn) {
                    toggleRouteBtn.checked = profile.is_routing_active;
                    toggleRouteBtn.addEventListener('change', (ev) => this.toggleRouting(ev.target.checked));
                }
        ...
        document.getElementById('agent-email-form')?.addEventListener('submit', async (e) => {
            ...
            try {
                await agentAPI.sendMessage(this.activeTicketId, text);
                input.value = ''; this.updateLocalBubble();
        ...
        if (btn && input) {
            ...
            input.addEventListener('change', async (e) => {
                ...
                try {
                    ...
                    this.updateLocalBubble();
                    if (isEmail) {
                        const msgs = await agentAPI.getMessages(this.activeTicketId); this.renderEmailThread(this.currentTicket, msgs);
                    } else { this.renderMsg("📎 Anexo enviado:", 'agent', fileData.url, fileData.name, fileData.type); }
    ...
    startLiveTimers() {
        ...
        this.timerInterval = setInterval(() => {
            ...
            // NOVO: REGRAS DE ENCERRAMENTO AUTOMÁTICO E FADING DA BOLHA
                ...
                this.activeTickets.forEach(t => {
                    ...
                    if (el) {
                        if (t.last_sender === 'customer') {
                            ...
                            // Clareia gradualmente usando rgba (de 1.0 até 0.15)
                            const opacity = Math.max(0.15, 1 - (diffMinutes / maxFadingMins));
                            el.style.backgroundColor = `rgba(37, 99, 235, ${opacity})`;
                            // Muda a cor da letra para não sumir no fundo muito claro
    ...
    async executeAutoClose(ticket) {
        try {
            ...
            await agentAPI.closeTicket(ticket.id, 'Encerrado Automaticamente (SLA)');
    ...
    async saveSettings() {
        ...
        try {
            ...
            this.systemSettings = payload; // Atualiza na memória local
    ...
    async loadQueue() {
        ...
        this.renderBubbles();
        let tickets = [];
        if (isGestor) { tickets = this.activeTickets; } 
        else {
        ...
        tbody.innerHTML = tickets.map(t => {
            ...
            let actionBtn = `<button onclick="agentApp.pickTicket('${t.id}')" class="bg-slate-900 text-white px-5 py-2.5 rounded-xl text-xs font-black hover:bg-blue-600 transition-all relative z-30">${inProg && isMine ? 'Retomar Chat' : 'Atender'}</button>`;
    ...
    async pickTicket(id) {
        try {
            ...
            if (t.channel === 'email') {
                ...
                document.getElementById('chat-header-name').innerText = t.customers?.full_name || 'Desconhecido'; document.getElementById('chat-header-protocol').innerText = `HZ-${t.protocol_number}`; document.getElementById('chat-header-channel').innerHTML = `<span class="material-symbols-outlined text-[14px]">mail</span> E-MAIL`; document.getElementById('chat-header-subject').innerText = `• ${t.ticket_subjects?.label || 'Sem assunto'}`;
                ...
                document.getElementById('chat-history').innerHTML = ''; document.getElementById('toggle-upload').checked = t.is_upload_enabled || false; this.updateUploadToggleUI(t.is_upload_enabled || false);
                document.getElementById('chat-header-name').innerText = t.customers?.full_name || 'Desconhecido'; document.getElementById('chat-header-protocol').innerText = `HZ-${t.protocol_number}`; document.getElementById('chat-header-channel').innerHTML = `<span class="material-symbols-outlined text-[14px]">forum</span> CHAT WEB`; document.getElementById('chat-header-subject').innerText = `• ${t.ticket_subjects?.label || 'Sem assunto'}`;
    ...
    renderEmailThread(ticket, msgs) {
        const area = document.getElementById('email-thread'); if (!area) return;
        let html = '';
        if (msgs.length > 0) {
        ...
        if (msgs.length > 1) {
            html += `<div class="flex items-center gap-4 my-6"><div class="h-px bg-slate-200 flex-1"></div><span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Histórico</span><div class="h-px bg-slate-200 flex-1"></div></div>`;
            for (let i = 1; i < msgs.length; i++) {
                const m = msgs[i]; const isAgent = m.sender_type === 'agent'; const dateStr = new Date(m.created_at).toLocaleString('pt-BR'); const senderName = isAgent ? 'Nossa Equipe (Analista)' : (ticket.customers?.full_name || 'Cliente'); const bgColor = isAgent ? 'bg-blue-50 border-blue-100' : 'bg-white border-slate-200';
