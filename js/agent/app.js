...
const App = {
    ...
    async init() {
                ...
                agentAPI.subscribeToMyProfile(this.currentUser.id, async (newProfile) => {
                    if (newProfile.status !== this.currentUser.status) {
                        ...
                        if (newProfile.status !== 'online') {
                            ...
                            this.navigate('queue'); alert("Atenção: Seu status foi alterado para " + newProfile.status.toUpperCase() + ".");
        ...
        document.getElementById('agent-email-form')?.addEventListener('submit', async (e) => {
            ...
            btn.innerHTML = `<span class="material-symbols-outlined animate-spin notranslate text-[18px]">refresh</span> Enviando...`; btn.disabled = true;
            try {
        ...
        document.getElementById('internal-chat-form')?.addEventListener('submit', async (e) => {
            ...
            try { await agentAPI.sendInternalMessage(this.currentUser.id, this.internalChatTarget, text); this.renderInternalMsg({ content: text }, true); input.value = ''; } catch (err) { alert("Erro ao enviar."); }
        });
    },

    // Tradução de interface (API Google Injetada)
    changeLanguage(lang) {
    ...
    // Tradução exclusiva das mensagens dinâmicas
    ...
    async executeAutoClose(ticket) {
        try {
            ...
            await this.loadQueue();
        } catch(e) { console.error("Falha macro:", e); } finally { this.closingTickets.delete(ticket.id); }
    },
    ...
    toggleAuthMode() {
        ...
        document.getElementById('btn-login').innerHTML = this.isRegisterMode ? 'Criar Conta' : 'Entrar <span class="material-symbols-outlined notranslate">login</span>';
    ...
    renderSettingsTags() {
        ...
        this.allSubjects.forEach(sub => {
            ...
            let subHtml = subsubs.map(ss => `<span class="bg-blue-50 text-blue-600 border border-blue-200 text-[10px] px-2 py-1 rounded font-bold flex items-center gap-1">${sub.label} - ${ss.label} <button onclick="agentApp.toggleSubsubject('${ss.id}', false)" class="hover:text-red-500 transition-colors flex items-center"><span class="material-symbols-outlined notranslate text-[12px]">close</span></button></span>`).join('');
    ...
    async saveSettings() {
        ...
        } catch (e) { alert("Erro ao salvar."); } finally { btn.innerHTML = origHtml; }
    },

    async toggleClientUpload(isEnabled) { 
        try { await agentAPI.toggleUpload(this.activeTicketId, isEnabled); this.updateUploadToggleUI(isEnabled); } catch(e) { alert("Erro."); document.getElementById('toggle-upload').checked = !isEnabled; } 
    },
    ...
    async loadQueue() {
        try {
            ...
            if (isGestor) {
                ...
                if (tickets.length === 0) {
                    tbody.innerHTML = `<tr><td colspan="4" class="p-10 text-center text-slate-300 font-bold">Nenhum ticket na fila.</td></tr>`; return;
                }
                ...
                if (this.currentUser.status !== 'online') { msgEl.innerHTML = `Você está <strong class="uppercase">${this.currentUser.status}</strong>.<br>Mude para ONLINE para receber chamados.`; } else { msgEl.innerHTML = `O Orquestrador enviará novos chamados automaticamente.<br>Fique atento às bolhas no cabeçalho da tela.`; }
            }
    ...
    async monitorTicket(ticketId, protocolNumber) {
        ...
        try {
            ...
            this.monitorSub = agentAPI.subscribeToAllMessages(ticketId, async (msgText, senderType, createdAt, fUrl, fName, fType) => { 
                if(senderType === 'customer') msgText = await this.fetchTranslation(msgText);
    ...
    async pickTicket(id) {
        try {
            ...
            if (t.status === 'open' || !t.agent_id) {
                ...
                if (myCount >= 10) { alert("Limite máximo de atendimentos simultâneos alcançado!"); this.navigate('queue'); return; }
            ...
            if (isEmail) {
                ...
                this.messageSub = agentAPI.subscribeToMessages(id, async (msgText, fUrl, fName, fType) => {
                    this.playAlert('message'); 
                    msgText = await this.fetchTranslation(msgText);
        ...
        } catch (e) { alert("Erro ao carregar os dados do chat."); }
    },
    ...
    renderMsg(text, type, fileUrl = null, fileName = null, fileType = null) {
        ...
        <div class="flex ${isAgent ? 'justify-end' : 'justify-start'} w-full notranslate">
            <div class="max-w-[80%] p-4 rounded-2xl text-sm font-medium shadow-sm ${type === 'system' ? 'bg-slate-200 text-slate-700 italic border border-slate-300 text-center w-full max-w-full rounded-2xl my-2' : (isAgent ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-white text-slate-800 rounded-tl-none border border-slate-100')} relative z-30 whitespace-pre-wrap">${type === 'system' ? '<span class="material-symbols-outlined notranslate text-[14px] align-middle mr-1">info</span>' : ''}${text}${mediaHtml}</div>
        </div>`;
    ...
    async viewPastChat(ticketId, protocolNumber) {
        try {
            ...
            <div class="bg-blue-50 border border-blue-100 p-4 rounded-xl mb-6 relative z-20"><h4 class="text-[10px] font-black text-blue-800 uppercase tracking-widest mb-3 flex items-center gap-1"><span class="material-symbols-outlined notranslate text-[14px]">summarize</span> Resumo do Atendimento</h4><div class="grid grid-cols-2 gap-4 text-xs mb-3"><div><div class="font-bold text-blue-900 mb-1">Motivo (Tag 1):</div><div class="text-blue-700 bg-white border border-blue-100 px-2 py-1 rounded font-bold inline-block">${ticket.agent_tag1 || 'Não classificado'}</div></div><div><div class="font-bold text-blue-900 mb-1">Submotivo (Tag 2):</div><div class="text-blue-700 bg-white border border-blue-100 px-2 py-1 rounded font-bold inline-block">${ticket.agent_tag2 || 'Não classificado'}</div></div></div><div class="text-xs"><div class="font-bold text-blue-900 mb-1">Observações do Analista:</div><div class="text-blue-800 italic bg-white/60 border border-blue-100 p-3 rounded-lg">${ticket.agent_notes || 'Nenhuma observação registrada.'}</div></div></div><div class="h-px bg-slate-200 mb-4"></div>
            `;
            if(msgs.length === 0) { html += '<div class="text-center text-slate-400 font-bold">Não existem mensagens registradas.</div>'; } 
            else { html += msgs.map(m => this.formatMonitorMsg(m.content, m.sender_type, m.created_at, m.file_url, m.file_name, m.file_type)).join(''); }
            content.innerHTML = html; content.scrollTop = content.scrollHeight;
        } catch (e) { console.error(e); alert("Erro ao abrir a conversa."); }
    },
    ...
    filterTeamTable() { this.teamSearchQuery = document.getElementById('team-search').value.toLowerCase(); this.renderTeamTable(); },
    ...
    async openInternalChat(targetId = null, targetName = null) {
        ...
        try {
            if (!this.internalChatTarget) throw new Error("ID não fornecido.");
    ...
    async approveMember(id) { if(confirm("Aprovar analista?")) { await agentAPI.approveUser(id, 'analista'); this.loadTeam(); } },
