...
const App = {
    activeTicketId: null, messageSub: null, monitorSub: null, internalChatTarget: null, isRegisterMode: false,
    currentUser: null, currentCustomer: null, currentTicket: null, 
    allSubjects: [], activeAgents: [], timerInterval: null, activeTickets: [],
    systemSettings: null, closingTickets: new Set(),
    ...
    async init() {
        ...
        // ===============================================
        // LOGIN (AGORA COM RASTREAMENTO DETALHADO DE ERRO)
        // ===============================================
        document.getElementById('login-form').addEventListener('submit', async (e) => {
            ...
            if (this.isRegisterMode) {
                try { 
                    ...
                    this.toggleAuthMode(); 
                } catch (err) { alert("Erro ao registrar: " + err.message); } finally { btn.innerHTML = originalText; }
                return;
            }
            ...
            try {
                ...
                console.log("Carregando assuntos e agentes...");
                ...
                agentAPI.subscribeToQueue(() => {
                    ...
                    if (document.getElementById('sec-dashboard') && !document.getElementById('sec-dashboard').classList.contains('hidden-view')) this.renderDashboard();
                    if (document.getElementById('sec-team') && !document.getElementById('sec-team').classList.contains('hidden-view')) this.loadTeam();
                });

                agentAPI.subscribeToSettings((newCfg) => {
                    this.systemSettings = newCfg;
                    Orchestrator.updateSettings(newCfg);
                    if (profile.role === 'gestor') { const tg = document.getElementById('toggle-routing'); if (tg) tg.checked = newCfg.is_orchestrator_active; }
                });

                agentAPI.subscribeToProfiles(() => { if (!document.getElementById('sec-team').classList.contains('hidden-view')) this.loadTeam(); });
        ...
        document.getElementById('agent-chat-form')?.addEventListener('submit', async (e) => {
            ...
            try { await agentAPI.sendMessage(this.activeTicketId, text); this.updateLocalBubble(); } catch(err) { alert("Erro ao enviar."); }
        });
        ...
        document.getElementById('toggle-upload')?.addEventListener('change', (e) => { this.toggleClientUpload(e.target.checked); });
        ...
        document.getElementById('agent-status-select')?.addEventListener('change', (e) => { this.updateMyStatus(e.target.value); });
    ...
    async toggleGlobalOrchestrator(isActive) {
        try { await agentAPI.updateSystemSettings({ is_orchestrator_active: isActive }); } catch (e) { document.getElementById('toggle-routing').checked = !isActive; alert("Erro ao alterar Orquestrador."); }
    },
    ...
    async updateMyStatus(newStatus) {
        ...
        try {
            ...
            if (newStatus !== 'online') {
                ...
                this.navigate('queue'); this.loadQueue();
            } else { Orchestrator.setStatus(true); this.loadQueue(); }
    ...
    async forceAgentStatus(agentId, newStatus) {
        ...
        try { await agentAPI.changeStatus(agentId, newStatus); if (newStatus !== 'online') await agentAPI.releaseMyTickets(agentId); this.loadTeam(); } catch(e) { alert("Erro ao alterar status do agente."); }
    },
    ...
    startLiveTimers() {
        ...
        this.timerInterval = setInterval(() => {
            ...
            if (this.systemSettings && this.currentUser) {
                const chatLimitSecs = this.systemSettings.chat_timeout_min * 60; const emailLimitSecs = this.systemSettings.email_timeout_hr * 3600;
                this.activeTickets.forEach(t => {
                    ...
                    if (el) {
                        if (t.last_sender === 'customer') {
                            el.style.backgroundColor = '#2563eb'; el.style.color = '#ffffff'; el.classList.add('animate-pulse');
                        } else {
                            el.classList.remove('animate-pulse');
                            const diffMinutes = Math.floor(diffSeconds / 60); const maxFadingMins = t.channel === 'web' ? this.systemSettings.chat_timeout_min : 60; 
                            const opacity = Math.max(0.15, 1 - (diffMinutes / maxFadingMins));
                            el.style.backgroundColor = `rgba(37, 99, 235, ${opacity})`; el.style.color = opacity < 0.4 ? '#1e3a8a' : '#ffffff';
                        }
                    }

                    if (t.last_sender === 'agent' && !this.closingTickets.has(t.id)) {
                        const isWebClose = t.channel === 'web' && diffSeconds >= chatLimitSecs; const isEmailClose = t.channel === 'email' && diffSeconds >= emailLimitSecs;
                        if (isWebClose || isEmailClose) { this.closingTickets.add(t.id); this.executeAutoClose(t); }
                    }
                });
            }
        }, 1000);
    ...
    async executeAutoClose(ticket) {
        try {
            ...
            msg = msg.replace(/\[nome do cliente\]/g, ticket.customers?.full_name || 'Cliente'); msg = msg.replace(/\[protocolo\]/g, ticket.protocol_number);
            await agentAPI.sendMessage(ticket.id, msg); await agentAPI.closeTicket(ticket.id, 'Encerrado Automaticamente (SLA Inatividade)');
            this.activeTickets = this.activeTickets.filter(t => t.id !== ticket.id);
            if (this.activeTicketId === ticket.id) { this.activeTicketId = null; document.getElementById('menu-chat').classList.add('hidden-view'); this.navigate('queue'); }
            this.loadQueue();
    ...
    async saveSettings() {
        const btn = document.getElementById('btn-save-cfg'); btn.innerHTML = `<span class="material-symbols-outlined animate-spin">refresh</span> Salvando...`;
        try {
            const payload = { max_chat_tickets: parseInt(document.getElementById('cfg-max-chat').value), max_email_tickets: parseInt(document.getElementById('cfg-max-email').value), chat_timeout_min: parseInt(document.getElementById('cfg-chat-time').value), email_timeout_hr: parseInt(document.getElementById('cfg-email-time').value), closure_macro: document.getElementById('cfg-macro').value };
            await agentAPI.updateSystemSettings(payload); this.systemSettings = payload; alert("Configurações atualizadas!");
        } catch (e) { alert("Erro ao salvar."); } finally { btn.innerHTML = `<span class="material-symbols-outlined">save</span> Salvar Configurações`; }
    },

    async toggleClientUpload(isEnabled) { try { await agentAPI.toggleUpload(this.activeTicketId, isEnabled); this.updateUploadToggleUI(isEnabled); } catch(e) { alert("Erro de permissão."); document.getElementById('toggle-upload').checked = !isEnabled; } },
    ...
    async monitorTicket(ticketId, protocolNumber) {
        ...
        document.getElementById('modal-monitor-protocol').innerText = `Protocolo HZ-${protocolNumber}`; modal.classList.remove('hidden-view'); content.innerHTML = '<div class="text-center text-slate-400 font-bold mt-4">Carregando...</div>';
        try {
    ...
    formatMonitorMsg(text, type, createdAt, fileUrl = null, fileName = null, fileType = null) {
        const isAgent = type === 'agent'; const timeStr = createdAt ? new Date(createdAt).toLocaleString('pt-BR', { dateStyle: 'short', timeStyle: 'short' }) : ''; const mediaHtml = this.buildMediaHtml(fileUrl, fileName, fileType);
    ...
    async pickTicket(id) {
        try {
            ...
            if (t.status === 'open' || !t.agent_id) {
                ...
                if (myCount >= 10) { alert("Limite alcançado!"); this.navigate('queue'); return; }
            ...
            document.getElementById('crm-name').innerText = t.customers?.full_name || 'Desconhecido'; document.getElementById('crm-email').innerText = t.customers?.email || 'Sem e-mail'; document.getElementById('crm-tag1').innerText = t.ticket_subjects?.label || 'Sem assunto'; document.getElementById('crm-tag2').value = t.tag2_detail || '';
        ...
        } catch (e) { alert("Erro ao carregar dados do chat: " + e.message); }
    },
    ...
    renderEmailThread(ticket, msgs) {
        ...
        if (msgs.length > 1) {
            ...
            for (let i = 1; i < msgs.length; i++) {
                const m = msgs[i]; const isAgent = m.sender_type === 'agent'; const dateStr = new Date(m.created_at).toLocaleString('pt-BR'); const senderName = isAgent ? 'Nossa Equipe' : (ticket.customers?.full_name || 'Cliente'); const bgColor = isAgent ? 'bg-blue-50 border-blue-100' : 'bg-white border-slate-200';
    ...
    renderMsg(text, type, fileUrl = null, fileName = null, fileType = null) {
        const isAgent = type === 'agent'; const mediaHtml = this.buildMediaHtml(fileUrl, fileName, fileType);
    ...
    async loadCustomerOrders(customerId) {
        try {
            ...
            if(orders.length === 0) { container.innerHTML = '<div class="text-xs text-slate-400 font-bold">Nenhum pedido.</div>'; return; }
    ...
    async renderDashboard() {
        try {
            const { data: tickets } = await agentAPI.getPendingTickets(); const { data: orders } = await supabase.from('orders').select('amount'); const { data: profiles } = await supabase.from('profiles').select('id, full_name');
    ...
    async loadTeam() {
        const team = await agentAPI.getTeamProfiles(); const logs = await agentAPI.getAgentLogsToday(); const tbody = document.getElementById('team-tbody');
        tbody.innerHTML = team.map(member => {
            ...
            if (member.is_approved) {
                ...
                skillsHTML = `<div class="flex flex-col gap-2"><div class="flex gap-4 border-b pb-2 border-slate-100"><label class="flex items-center gap-1 cursor-pointer"><input type="checkbox" ${member.can_web ? 'checked' : ''} onchange="agentApp.toggleChannel('${member.id}', 'web', this.checked)" class="w-3 h-3 text-blue-600"><span class="text-[10px] font-black text-slate-600">WEB (Chat)</span></label><label class="flex items-center gap-1 cursor-pointer"><input type="checkbox" ${member.can_email ? 'checked' : ''} onchange="agentApp.toggleChannel('${member.id}', 'email', this.checked)" class="w-3 h-3 text-blue-600"><span class="text-[10px] font-black text-slate-600">E-MAIL</span></label></div><div class="flex flex-wrap gap-1 min-h-[24px]">${skillsBadges || '<span class="text-[10px] text-slate-400 font-bold italic">Nenhum assunto</span>'}</div><select onchange="if(this.value) { agentApp.toggleSkill('${member.id}', this.value, true); this.value=''; }" class="bg-slate-50 border border-slate-200 text-slate-600 text-[10px] font-bold rounded p-1.5 outline-none w-full cursor-pointer hover:bg-slate-100 transition-colors">${availableOptions}</select></div>`;
            }
    ...
    async toggleChannel(agentId, channel, isEnabled) { try { await agentAPI.toggleChannel(agentId, channel, isEnabled); if (!isEnabled) { await agentAPI.releaseTicketsByChannel(agentId, channel); } this.loadTeam(); } catch (e) { console.error(e); this.loadTeam(); } },
    async toggleSkill(agentId, subjectId, isAdding) { try { await agentAPI.toggleAgentSkill(agentId, subjectId, isAdding); this.loadTeam(); } catch (e) { console.error(e); this.loadTeam(); } }
};
