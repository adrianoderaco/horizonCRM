...
const App = {
    ...
    init() {
        ...
        // Eventos Manuais Seguros
        const toggleUpload = document.getElementById('toggle-upload');
        if (toggleUpload) toggleUpload.addEventListener('change', (e) => this.toggleClientUpload(e.target.checked));
        ...
        if (btnAgentAttach && agentFileInput) {
            ...
            agentFileInput.addEventListener('change', async (e) => {
                ...
                const btnSend = document.getElementById('btn-send'); const origText = btnSend.innerHTML;
    ...
    applyWatermark(name) {
        ...
        wm.innerHTML = Array(150).fill(`<span class="transform -rotate-45 text-2xl font-black text-slate-900 m-8 opacity-[0.03]">${name}</span>`).join('');
    ...
    navigate(target) {
        ['queue', 'chat', 'team', 'dashboard'].forEach(s => { const el = document.getElementById(`sec-${s}`); if(el) el.classList.add('hidden-view'); });
    ...
    async loadQueue() {
        ...
        if (isGestor) { tickets = this.activeTickets; } 
            ...
            // Analista vê seus tickets em curso OU tickets abertos que sejam de Canais que ele tem permissão
    ...
    async loadTeam() {
        ...
        tbody.innerHTML = team.map(member => {
            ...
            return `<tr class="relative z-20"><td class="p-5 font-black text-slate-900">${member.full_name}</td><td class="p-5">${skillsHTML}</td><td class="p-5 text-right">${!member.is_approved ? `<button onclick="agentApp.approveMember('${member.id}')" class="bg-blue-600 text-white px-4 py-2 rounded font-bold text-xs">Aprovar</button>` : ''}</td></tr>`;
        }).join('');
    },
