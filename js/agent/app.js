...
const App = {
    ...
    init() {
        ...
        document.getElementById('login-form').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                // Ajusta o dropdown de status para o valor atual
        ...
        // Evento Protegido do Toggle de Upload
        document.getElementById('toggle-upload')?.addEventListener('change', (e) => {
            this.toggleClientUpload(e.target.checked);
        });

        // Evento Protegido de Envio de Arquivo do Agente
        const agentFileInput = document.getElementById('agent-file-input');
        const btnAgentAttach = document.getElementById('btn-agent-attach');
        
        if (btnAgentAttach && agentFileInput) {
            btnAgentAttach.addEventListener('click', () => agentFileInput.click());
    ...
    async updateMyStatus(newStatus) {
        ...
        try {
            ...
            // Se o agente sair de "Online", devolve as conversas e trava o orquestrador
            if (newStatus !== 'online') {
                ...
                // Redireciona ele para a tela de monitor e limpa o chat
    ...
    applyWatermark(name) {
        ...
        // z-index altíssimo mas sem bloquear clique (pointer-events-none)
        ...
        let spans = '';
        for(let i=0; i<150; i++) {
            spans += `<span class="transform -rotate-45 text-2xl font-black text-slate-900 m-8" style="opacity: 0.03;">${name}</span>`;
        }
        wm.innerHTML = spans;
    ...
    async logout() {
        if(confirm("Deseja sair? Seus atendimentos em andamento voltarão para a fila!")) {
            try {
                await agentAPI.releaseMyTickets(this.currentUser.id);
                await agentAPI.changeStatus(this.currentUser.id, 'offline'); // Loga como offline
                await supabase.auth.signOut();
                location.reload();
            } catch(e) {
    ...
    navigate(target) {
        ['queue', 'chat', 'team', 'dashboard'].forEach(s => {
    ...
    async monitorTicket(ticketId, protocolNumber) {
        ...
        try {
            ...
            content.innerHTML = '<div class="text-center text-red-400 font-bold mt-4">Erro ao carregar monitoria.</div>';
        }
    },
    ...
    async pickTicket(id) {
        try {
            ...
            const chIcon = t.channel === 'email' ? 'mail' : 'forum';
            const chText = t.channel === 'email' ? 'E-MAIL' : 'CHAT WEB';
            ...
            document.getElementById('chat-header-channel').innerHTML = `<span class="material-symbols-outlined text-[14px]">${chIcon}</span> ${chText}`;
    ...
    renderMsg(text, type, fileUrl = null, fileName = null, fileType = null) {
        ...
        if (fileUrl) {
            if (fileType && fileType.startsWith('image/')) {
                ...
                mediaHtml = `<div class="mt-2"><a href="${fileUrl}" target="_blank" download class="flex items-center gap-2 bg-black/10 p-2.5 rounded-lg text-xs font-bold hover:bg-black/20 transition-colors cursor-pointer relative z-30"><span class="material-symbols-outlined text-sm">download</span> ${fileName || 'Arquivo'}</a></div>`;
            }
        }
    ...
    populateTransferDropdowns() {
        ...
        this.activeAgents.forEach(a => {
            if (a.id !== this.currentUser.id) {
                agSelect.innerHTML += `<option value="${a.id}">${a.full_name} (Online)</option>`;
            }
        });
    },
        ...
        try {
            ...
            container.innerHTML = hist.map(h => `
                <div class="p-3 bg-slate-50 border rounded-xl flex justify-between items-center transition-all hover:border-blue-300 relative z-20">
                    ...
                    <button onclick="agentApp.viewPastChat('${h.id}', '${h.protocol_number}')" title="Ver Conversa" class="w-8 h-8 flex items-center justify-center bg-white border rounded-lg hover:bg-blue-50 text-slate-400 hover:text-blue-600 shadow-sm transition-all">
    ...
    async loadCustomerOrders(customerId) {
        try {
            ...
            if(orders.length === 0) {
                container.innerHTML = '<div class="text-xs text-slate-400 font-bold">Nenhum pedido registrado.</div>';
                return;
            }
    ...
    // GESTÃO DE EQUIPE (CANAIS E RELATÓRIO DE TEMPO)
    async loadTeam() {
        ...
        tbody.innerHTML = team.map(member => {
            ...
            // --- MATEMÁTICA DE TEMPO (PONTO DO DIA) ---
            ...
            // Soma o tempo do status atual que ainda está rodando
            ...
            // Formatação do Badge de Status Atual
            let statusBadge = '';
            if(member.status === 'online') statusBadge = '<span class="bg-green-100 text-green-700 px-2 py-0.5 rounded font-bold text-[10px]">ONLINE</span>';
            else if(member.status === 'pausa') statusBadge = '<span class="bg-amber-100 text-amber-700 px-2 py-0.5 rounded font-bold text-[10px]">EM PAUSA</span>';
            else if(member.status === 'backoffice') statusBadge = '<span class="bg-purple-100 text-purple-700 px-2 py-0.5 rounded font-bold text-[10px]">BACKOFFICE</span>';
            else statusBadge = '<span class="bg-slate-100 text-slate-500 px-2 py-0.5 rounded font-bold text-[10px]">OFFLINE</span>';

            const timeHtml = `
                <div class="mb-2">${statusBadge}</div>
                <div class="text-[10px] text-slate-500 font-bold space-y-0.5">
                    <div><span class="text-green-600">Online:</span> ${fmtTime(onlineSecs)}</div>
                    <div><span class="text-amber-600">Pausa:</span> ${fmtTime(pausaSecs)}</div>
                    <div><span class="text-purple-600">Backoffice:</span> ${fmtTime(backSecs)}</div>
                </div>
            `;
            ...
            if (member.is_approved) {
                skillsHTML = `<div class="flex flex-col gap-1">
                    <div class="flex gap-4 mb-2 border-b pb-2 border-slate-100">
                ...
                this.allSubjects.forEach(sub => {
                    ...
                    skillsHTML += `<label class="flex items-center gap-2 cursor-pointer w-fit"><input type="checkbox" ${hasSkill ? 'checked' : ''} onchange="agentApp.toggleSkill('${member.id}', '${sub.id}', this.checked)" class="w-4 h-4 text-blue-600"><span class="text-[10px] font-bold text-slate-500">${sub.label}</span></label>`;
                });
                skillsHTML += `</div>`;
            }
            ...
            <tr class="relative z-20 hover:bg-slate-50 transition-colors">
                <td class="p-5 font-black text-slate-900">${member.full_name}<div class="text-[10px] font-bold text-slate-400 mt-1 uppercase">${member.role}</div></td>
                <td class="p-5">${timeHtml}</td>
                <td class="p-5">${skillsHTML}</td>
                <td class="p-5 text-right">${!member.is_approved ? `<button onclick="agentApp.approveMember('${member.id}')" class="bg-blue-600 text-white px-4 py-2 rounded font-bold text-xs">Aprovar</button>` : ''}</td>
            </tr>`;
        }).join('');
    },
