...
const App = {
    ...
    async init() {
        ...
        try {
            ...
            console.warn("Aviso: Configuracoes ausentes. Usando locais.");
            this.systemSettings = {
                ...
                chat_warning_min: 8,
                email_warning_hr: 20,
                warning_macro_chat: "Ola, ainda esta ai?",
        ...
        document.getElementById('login-form')?.addEventListener('submit', async (e) => {
            ...
            if (agentApp.isRegisterMode) {
                try { await agentAPI.register(document.getElementById('reg-name').value, email, pass); alert("Aguarde aprovacao."); agentApp.toggleAuthMode(); }
            ...
            try {
                ...
                if (!profile || !profile.is_approved) { alert("Acesso pendente."); await supabase.auth.signOut(); btn.innerHTML = orig; return; }
                ...
                // Aplicar idioma salvo e configurar tradutor
                const savedLang = localStorage.getItem('horizon_agent_lang') || 'pt';
                ...
                if (profile.role === 'gestor') {
                    ...
                    await agentApp.renderDashboard();
                }
                ...
                agentAPI.subscribeToQueue(async () => {
                    ...
                    if (document.getElementById('sec-dashboard') && !document.getElementById('sec-dashboard').classList.contains('hidden-view')) await agentApp.renderDashboard();
                ...
                agentAPI.subscribeToInternalMessages(agentApp.currentUser.id, async (msg) => {
                    if (document.getElementById('modal-internal-chat').classList.contains('hidden-view') || agentApp.internalChatTarget !== msg.sender_id) {
                        ...
                        agentApp.renderInternalMsg(msg, false);
                    }
                });
        ...
        document.getElementById('agent-chat-form')?.addEventListener('submit', async (e) => {
            ...
            agentApp.renderMsg(text, 'agent');
            input.value = '';
            try { await agentAPI.sendMessage(agentApp.activeTicketId, text); agentApp.updateLocalBubble(); Orchestrator.findAndClaimNext(); }
        ...
        document.getElementById('agent-email-form')?.addEventListener('submit', async (e) => {
            ...
            const text = input.value.trim();
            if (!text || !agentApp.activeTicketId) return;
            if (!confirm("Prosseguir com a resposta ao cliente? O ticket sairah da sua tela.")) return;
            const btn = document.getElementById('btn-send-email');
            const origHTML = btn.innerHTML;
            btn.innerHTML = `<span class="material-symbols-outlined animate-spin text-[18px]">refresh</span> Enviando...`;
            btn.disabled = true;
            try {
                await agentAPI.sendMessage(agentApp.activeTicketId, text);
                input.value = '';
            ...
            } catch (err) { alert("Erro ao responder o e-mail."); }
        ...
        document.getElementById('internal-chat-form')?.addEventListener('submit', async (e) => {
            ...
            try { await agentAPI.sendInternalMessage(agentApp.currentUser.id, agentApp.internalChatTarget, text); agentApp.renderInternalMsg({ content: text }, true); input.value = ''; }
            catch (err) { alert("Erro ao enviar mensagem interna."); }
        });
    },

    // ===============================================
    // IDIOMA: traduz toda a tela estática via Translator
    // ===============================================
    changeLanguage(lang) {
        localStorage.setItem('horizon_agent_lang', lang);
    ...
    playAlert(type) {
        ...
        try {
            ...
            audio.play();
        } catch (e) { console.error("Erro ao tocar audio", e); }
    },

    async toggleGlobalOrchestrator(isActive) {
        try { await agentAPI.updateSystemSettings({ is_orchestrator_active: isActive }); }
        catch (e) { document.getElementById('toggle-routing').checked = !isActive; alert("Erro ao alterar Orquestrador Global."); }
    },
    ...
    setupAgentAttachment(btnId, inputId, isEmail = false) {
        ...
        if (btn && input) {
            ...
            input.addEventListener('change', async (e) => {
                ...
                if (!file || !agentApp.activeTicketId) return;
                ...
                try {
                    ...
                    if (isEmail) {
                        const msgs = await agentAPI.getMessages(agentApp.activeTicketId);
                        agentApp.renderEmailThread(agentApp.currentTicket, msgs);
                    } else {
                        agentApp.renderMsg("Anexo enviado:", 'agent', fileData.url, fileData.name, fileData.type);
                    }
                } catch (err) { alert("Erro no upload."); }
                finally { btnSend.innerHTML = origText; btnSend.disabled = false; e.target.value = ''; }
            });
        }
    },
    ...
    updateLocalBubble() {
        ...
        if (tkIndex > -1) {
            agentApp.activeTickets[tkIndex].last_sender = 'agent';
            agentApp.activeTickets[tkIndex].last_interaction_at = new Date().toISOString();
    ...
    // ===============================================
    // FORCAR STATUS DO AGENTE (gestor) — ativa orquestrador apos liberacao
    // ===============================================
    async forceAgentStatus(agentId, newStatus) {
        if (!confirm(`Deseja forcar o status para ${newStatus.toUpperCase()}? Os tickets em andamento serao devolvidos a fila.`)) return;
        try {
            await agentAPI.changeStatus(agentId, newStatus);
            if (newStatus !== 'online') {
                // Libera os tickets do agente de volta para a fila
                await agentAPI.releaseMyTickets(agentId);
            }
            // Aciona o orquestrador para redistribuir os tickets liberados
            Orchestrator.findAndClaimNext();
            await agentApp.loadTeam();
        } catch (e) { alert("Erro ao alterar status do agente."); }
    },
    ...
    async updateAgentLimits(agentId, maxChats, maxEmails) {
        ...
        } catch (e) { alert("Erro ao atualizar limites do agente."); }
    },

    async updateAgentGroup(agentId, groupName) {
        try { await agentAPI.updateAgentGroup(agentId, groupName); }
        catch (e) { alert("Erro ao atualizar equipe do agente."); }
    },
    ...
    async logout() {
        if (confirm("Deseja sair? Seus atendimentos em andamento voltarao para a fila!")) {
            try { await agentAPI.releaseMyTickets(agentApp.currentUser.id); await agentAPI.changeStatus(agentApp.currentUser.id, 'offline'); await supabase.auth.signOut(); location.reload(); }
            catch (e) { alert("Erro ao deslogar."); }
        }
    },

    // ===============================================
    // TIMERS E SLA — encerramento inteligente por inatividade
    // ===============================================
    startLiveTimers() {
                ...
                agentApp.activeTickets.forEach(t => {
                    ...
                    if (el) {
                        if (t.last_sender === 'customer') {
                            ...
                            if (!t.first_reply_at) { el.style.backgroundColor = '#16a34a'; el.style.color = '#ffffff'; }
                            else { el.style.backgroundColor = '#2563eb'; el.style.color = '#ffffff'; }
                        } else {
                    ...
                    if (t.last_sender === 'agent' && !agentApp.closingTickets.has(t.id)) {
                        if (!t.has_warning_sent) {
                            ...
                            if (isWebWarn || isEmailWarn) { t.has_warning_sent = true; agentApp.executeWarning(t); }
                        } else {
    ...
    async executeWarning(ticket) {
        try {
            ...
            let msg = rawMsg || "Atencao: O atendimento sera encerrado em breve por inatividade. Ainda esta por ai?";
        ...
        } catch (e) { console.error("Falha ao enviar alerta SLA:", e); }
    },

    // ===============================================
    // ENCERRAMENTO AUTOMATICO INTELIGENTE
    // <= 3 interacoes do cliente: fecha automaticamente com tag "Outros - Inatividade"
    // >  3 interacoes do cliente: notifica o agente para classificar e fechar manualmente
    // ===============================================
    async executeAutoClose(ticket) {
        try {
            console.log(`[SLA] Processando encerramento por inatividade: HZ-${ticket.protocol_number}`);

            let customerInteractionCount = 0;
            try {
                const msgs = await agentAPI.getMessages(ticket.id);
                customerInteractionCount = msgs.filter(m => m.sender_type === 'customer').length;
            } catch (e) { customerInteractionCount = 0; }

            let rawMsg = agentApp.systemSettings.closure_macro;
            let closureMsg = rawMsg || "Atendimento encerrado por inatividade.";
            closureMsg = closureMsg.replace(/\[nome do cliente\]/gi, ticket.customers?.full_name || 'Cliente');
            closureMsg = closureMsg.replace(/\[protocolo\]/gi, ticket.protocol_number);

            // Envia mensagem final de encerramento (visivelao cliente via realtime)
            await agentAPI.sendSystemMessage(ticket.id, closureMsg, null);

            if (customerInteractionCount <= 3) {
                // Encerra automaticamente com tag padrao
                console.log(`[SLA] ${customerInteractionCount} interacoes - encerrando automaticamente.`);
                await agentAPI.closeTicket(ticket.id, 'Outros', 'Inatividade', 'Encerrado pelo sistema (Falta de interacao do cliente).');
                ...
                // Notifica o agente para classificar e fechar manualmente
                console.log(`[SLA] ${customerInteractionCount} interacoes - notificando agente para encerrar manualmente.`);
                agentApp.playAlert('ticket');
                if (agentApp.activeTicketId === ticket.id) {
                    agentApp.renderMsg(closureMsg, 'system');
                    agentApp.showInactivityAgentModal(ticket);
                } else {
                    const el = document.getElementById(`bubble-${ticket.id}`);
                    if (el) {
                        el.style.backgroundColor = '#dc2626';
                        el.style.color = '#ffffff';
                        el.title = `Acao necessaria: HZ-${ticket.protocol_number} - classifique e feche o ticket!`;
                        el.classList.add('animate-pulse');
                    }
                }
            }
        } catch (e) { console.error("Falha no encerramento SLA:", e); }
        finally { agentApp.closingTickets.delete(ticket.id); }
    },

    showInactivityAgentModal(ticket) {
        const old = document.getElementById('modal-inactivity-alert');
        if (old) old.remove();
        const modal = document.createElement('div');
        modal.id = 'modal-inactivity-alert';
        modal.className = 'fixed inset-0 bg-slate-900/80 z-[200] flex items-center justify-center p-6';
        modal.innerHTML = `
        <div class="bg-white w-full max-w-md rounded-3xl flex flex-col shadow-2xl overflow-hidden">
            <header class="p-6 bg-amber-50 border-b border-amber-200">
                <div class="flex items-center gap-3">
                    <div class="w-12 h-12 bg-amber-100 text-amber-600 rounded-full flex items-center justify-center">
                        <span class="material-symbols-outlined text-2xl">warning</span>
                    </div>
                    <div>
                        <h3 class="font-black text-slate-900">Atendimento Inativo</h3>
                        <p class="text-xs font-bold text-amber-700 mt-1">HZ-${ticket.protocol_number} - Acao Necessaria</p>
                    </div>
                </div>
            </header>
            <div class="p-6 space-y-4">
                <p class="text-sm font-medium text-slate-700">
                    O atendimento foi encerrado por inatividade, mas o cliente teve <strong>mais de 3 interacoes</strong>. Por isso, e necessario classificar e fechar manualmente.
                </p>
                <div class="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-2 text-xs font-bold text-slate-500">
                    <div>1. Selecione o <span class="text-slate-800">Motivo (Tag 1)</span> e <span class="text-slate-800">Submotivo (Tag 2)</span></div>
                    <div>2. Adicione uma <span class="text-slate-800">Observacao</span></div>
                    <div>3. Clique em <span class="text-blue-700">Finalizar Atendimento</span></div>
                </div>
                <button onclick="document.getElementById('modal-inactivity-alert').remove(); agentApp.switchTab('crm-info');"
                    class="w-full bg-amber-500 hover:bg-amber-600 text-white font-bold py-3 rounded-xl transition-all flex items-center justify-center gap-2">
                    <span class="material-symbols-outlined">edit_note</span> Ir para Classificacao
                </button>
            </div>
        </div>`;
    ...
    renderBubbles() {
        ...
        if (!container) return;
        const myTickets = agentApp.activeTickets
            .filter(t => t.status === 'in_progress' && t.agent_id === agentApp.currentUser.id)
    ...
    toggleAuthMode() {
        ...
        if (agentApp.isRegisterMode) { regName.classList.remove('hidden-view'); regName.required = true; }
    ...
    navigate(target) {
        ...
        if (target === 'team') agentApp.loadTeam();
        if (target === 'dashboard') agentApp.renderDashboard();
        if (target === 'settings') agentApp.loadSettingsUI();
    },
    ...
    async loadSettingsUI() {
        ...
        } catch (e) { console.error("Erro UI de configuracoes", e); }
    },
    ...
    async addTag1() {
        ...
        if (!val) return;
        try { await agentAPI.createSubject(val); input.value = ''; agentApp.allSubjects = await agentAPI.getAllSubjects(); agentApp.renderSettingsTags(); }
    ...
    async addTag2(subjectId) {
        ...
        const val = input.value.trim();
        if (!val) return;
        try { await agentAPI.createSubsubject(subjectId, val); input.value = ''; agentApp.allSubsubjects = await agentAPI.getAllSubsubjects(); agentApp.renderSettingsTags(); }
    ...
    async toggleSubject(id, isActive) {
        ...
        try { await agentAPI.toggleSubject(id, isActive); agentApp.allSubjects = await agentAPI.getAllSubjects(); agentApp.renderSettingsTags(); }
        catch (e) { alert("Erro ao excluir."); }
    },

    async toggleSubsubject(id, isActive) {
        try { await agentAPI.toggleSubsubject(id, isActive); agentApp.allSubsubjects = await agentAPI.getAllSubsubjects(); agentApp.renderSettingsTags(); }
    ...
    async saveSettings() {
        ...
        } catch (e) { alert("Erro ao salvar as configuracoes."); }
        finally { btn.innerHTML = origHtml; }
    },

    async toggleClientUpload(isEnabled) {
        try { await agentAPI.toggleUpload(agentApp.activeTicketId, isEnabled); agentApp.updateUploadToggleUI(isEnabled); }
        catch (e) { alert("Erro de permissao ao ativar anexo."); document.getElementById('toggle-upload').checked = !isEnabled; }
    },
        ...
        try {
            ...
            if (isGestor) {
                ...
                tbody.innerHTML = tickets.map(t => {
                    ...
                    const isMine = t.agent_id === agentApp.currentUser.id;
                ...
                if (agentApp.currentUser.status !== 'online') {
                    msgEl.innerHTML = `Voce esta <strong class="uppercase">${agentApp.currentUser.status}</strong>.<br>Mude para ONLINE para receber chamados da fila.`;
                } else {
                    msgEl.innerHTML = `O Orquestrador enviara novos chamados automaticamente.<br>Fique atento as bolhas no cabecalho da sua tela.`;
                }
            }
    ...
    async reassignTicket(ticketId, newAgentId) {
        if (confirm("Deseja alterar o dono deste chamado?")) { await agentAPI.reassignTicket(ticketId, newAgentId); }
        else { await agentApp.loadQueue(); }
    },
    ...
    async monitorTicket(ticketId, protocolNumber) {
        ...
        try {
            ...
            let html = `<div class="bg-blue-50 border border-blue-100 p-4 rounded-xl mb-6 relative z-20"><h4 class="text-[10px] font-black text-blue-800 uppercase tracking-widest mb-3 flex items-center gap-1"><span class="material-symbols-outlined text-[14px]">summarize</span> Resumo do Atendimento</h4><div class="grid grid-cols-2 gap-4 text-xs mb-3"><div><div class="font-bold text-blue-900 mb-1">Motivo (Tag 1):</div><div class="text-blue-700 bg-white border border-blue-100 px-2 py-1 rounded font-bold inline-block">${ticket.agent_tag1 || 'Nao classificado'}</div></div><div><div class="font-bold text-blue-900 mb-1">Submotivo (Tag 2):</div><div class="text-blue-700 bg-white border border-blue-100 px-2 py-1 rounded font-bold inline-block">${ticket.agent_tag2 || 'Nao classificado'}</div></div></div><div class="text-xs"><div class="font-bold text-blue-900 mb-1">Observacoes do Analista:</div><div class="text-blue-800 italic bg-white/60 border border-blue-100 p-3 rounded-lg">${ticket.agent_notes || 'Nenhuma observacao registrada.'}</div></div></div><div class="h-px bg-slate-200 mb-4"></div>`;
            html += msgs.map(m => agentApp.formatMonitorMsg(m.content, m.sender_type, m.created_at, m.file_url, m.file_name, m.file_type, m.is_internal_note)).join('');
            content.innerHTML = html;
            content.scrollTop = content.scrollHeight;
            if (agentApp.monitorSub) agentApp.monitorSub.unsubscribe();
            agentApp.monitorSub = agentAPI.subscribeToAllMessages(ticketId, (msgText, senderType, createdAt, fUrl, fName, fType) => {
                content.innerHTML += agentApp.formatMonitorMsg(msgText, senderType, createdAt, fUrl, fName, fType);
                content.scrollTop = content.scrollHeight;
            });
        } catch (e) { content.innerHTML = '<div class="text-center text-red-400 font-bold mt-4">Erro de conexao.</div>'; }
    },

    // Nota interna de transferencia: renderiza como pichula roxa (nao aparece para o cliente)
    formatMonitorMsg(text, type, createdAt, fileUrl = null, fileName = null, fileType = null, isInternalNote = false) {
        if (isInternalNote) {
            return `<div class="w-full flex justify-center my-2">
                <div class="bg-violet-50 border border-violet-200 text-violet-700 text-[10px] font-bold px-4 py-2 rounded-full flex items-center gap-1 max-w-sm text-center">
                    <span class="material-symbols-outlined text-[12px]">swap_horiz</span>${text}
                </div>
            </div>`;
        }
        ...
        const mediaHtml = agentApp.buildMediaHtml(fileUrl, fileName, fileType);
        return `<div class="flex flex-col ${isAgent ? 'items-end' : 'items-start'} w-full mb-4"><div class="text-[9px] text-slate-400 font-bold mb-1 px-1">${type === 'system' ? 'Sistema Automatico' : (isAgent ? 'Analista' : 'Cliente')} - ${timeStr}</div><div class="max-w-[85%] p-3 rounded-xl text-xs font-medium shadow-sm whitespace-pre-wrap ${isAgent ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-white border border-slate-200 text-slate-800 rounded-tl-none'}">${text}${mediaHtml}</div></div>`;
    },
    ...
    async pickTicket(id) {
        try {
            ...
            if (t.status === 'open' || !t.agent_id) {
                ...
                if (myCount >= 10) { alert("O limite maximo de atendimentos simultaneos foi alcancado!"); agentApp.navigate('queue'); return; }
                await agentAPI.reassignTicket(id, agentApp.currentUser.id);
                t.status = 'in_progress';
                t.agent_id = agentApp.currentUser.id;
                await agentApp.loadQueue();
            }

            agentApp.renderBubbles();
            agentApp.currentCustomer = t.customers;

            document.getElementById('toggle-upload').checked = t.is_upload_enabled || false;
            agentApp.updateUploadToggleUI(t.is_upload_enabled || false);
            ...
            agentApp.activeAgents = await agentAPI.getActiveAgents();
            agentApp.populateTransferDropdowns();

            if (t.customers?.email) agentApp.loadCustomerHistory(t.customers.email);
            if (t.customer_id) agentApp.loadCustomerOrders(t.customer_id);
            ...
            if (isEmail) {
                ...
                // Renderiza historico com traducao automatica para mensagens do cliente
                for (const m of msgs) {
                    await agentApp.renderMsg(m.content, m.sender_type, m.file_url, m.file_name, m.file_type, m.is_internal_note);
                }

                agentApp.messageSub = agentAPI.subscribeToMessages(id, async (msgText, fUrl, fName, fType) => {
                    agentApp.playAlert('message');
                    await agentApp.renderMsg(msgText, 'customer', fUrl, fName, fType);
                    const tkIndex = agentApp.activeTickets.findIndex(tk => tk.id === id);
                    if (tkIndex > -1) {
                        agentApp.activeTickets[tkIndex].last_sender = 'customer';
                        agentApp.activeTickets[tkIndex].last_interaction_at = new Date().toISOString();
                        agentApp.renderBubbles();
                    }
                });
    ...
    renderEmailThread(ticket, msgs) {
        ...
        if (msgs.length > 0) {
            ...
            html += `<div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative z-30"><div class="flex justify-between items-start border-b border-slate-100 pb-4 mb-4"><div><div class="font-black text-slate-900 text-lg">${ticket.customers?.full_name || 'Cliente'}</div><div class="text-sm font-bold text-slate-500">&lt;${ticket.customers?.email || 'Sem e-mail'}&gt;</div></div><div class="text-xs font-bold text-slate-400">${dateStr}</div></div><div class="text-sm text-slate-800 whitespace-pre-wrap font-medium leading-relaxed">${firstMsg.content}</div>${agentApp.buildMediaHtml(firstMsg.file_url, firstMsg.file_name, firstMsg.file_type)}</div>`;
        }
        if (msgs.length > 1) {
            html += `<div class="flex items-center gap-4 my-6"><div class="h-px bg-slate-200 flex-1"></div><span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Historico da Thread</span><div class="h-px bg-slate-200 flex-1"></div></div>`;
            for (let i = 1; i < msgs.length; i++) {
                const m = msgs[i];
                // Nota de transferencia: pill roxa visivel apenas ao agente
                if (m.is_internal_note) {
                    html += `<div class="w-full flex justify-center my-2"><div class="bg-violet-50 border border-violet-200 text-violet-700 text-[10px] font-bold px-4 py-2 rounded-full flex items-center gap-1"><span class="material-symbols-outlined text-[12px]">swap_horiz</span>${m.content}</div></div>`;
                    continue;
                }
                const isAgent = m.sender_type === 'agent' || m.sender_type === 'system';
                const dateStr = new Date(m.created_at).toLocaleString('pt-BR');
                const senderName = m.sender_type === 'system' ? 'Sistema Automatico' : (isAgent ? 'Nossa Equipe (Analista)' : (ticket.customers?.full_name || 'Cliente'));
                const bgColor = isAgent ? 'bg-blue-50 border-blue-100' : 'bg-white border-slate-200';
                html += `<div class="p-5 rounded-2xl border ${bgColor} shadow-sm relative z-30 mb-4 ${isAgent ? 'ml-12' : 'mr-12'}"><div class="flex justify-between items-center mb-3 border-b ${isAgent ? 'border-blue-100' : 'border-slate-100'} pb-2"><span class="text-xs font-black ${isAgent ? 'text-blue-700' : 'text-slate-700'}">${senderName}</span><span class="text-[10px] font-bold text-slate-400">${dateStr}</span></div><div class="text-sm text-slate-700 whitespace-pre-wrap">${m.content}</div>${agentApp.buildMediaHtml(m.file_url, m.file_name, m.file_type)}</div>`;
            }
        }
        area.innerHTML = html;
    ...
    // ===============================================
    // RENDERIZAR MENSAGEM com traducao automatica do cliente
    // ===============================================
    async renderMsg(text, type, fileUrl = null, fileName = null, fileType = null, isInternalNote = false) {
        const area = document.getElementById('chat-history');
        if (!area) return;

        // Nota interna de transferencia — pill roxa, so visivel ao agente
        if (isInternalNote) {
            area.innerHTML += `<div class="w-full flex justify-center my-2">
                <div class="bg-violet-50 border border-violet-200 text-violet-700 text-[10px] font-bold px-4 py-2 rounded-full flex items-center gap-1">
                    <span class="material-symbols-outlined text-[12px]">swap_horiz</span>${text}
                </div>
            </div>`;
            area.scrollTop = area.scrollHeight;
        ...
        // Traducao automatica: se o agente usa outro idioma, traduz mensagens do cliente
        let displayText = text;
        if (type === 'customer' && Translator.currentLang !== 'pt') {
            try {
                const translated = await Translator.translateDynamic(text);
                if (translated && translated !== text) {
                    displayText = `${translated}<br><span style="font-size:10px;opacity:0.55;font-style:italic;">Original: ${text}</span>`;
                }
            } catch (e) { /* mantém texto original em caso de falha */ }
        }
        ...
        if (fileUrl) {
            if (fileType && fileType.startsWith('image/')) {
                ...
                mediaHtml = `<div class="mt-2"><a href="${fileUrl}" target="_blank" download class="flex items-center gap-2 bg-black/10 p-2.5 rounded-lg text-xs font-bold w-max max-w-full truncate"><span class="material-symbols-outlined text-sm shrink-0">download</span> <span class="truncate">${fileName || 'Arquivo anexado'}</span></a></div>`;
            }
        }

        area.innerHTML += `<div class="flex ${isAgent ? 'justify-end' : 'justify-start'} w-full"><div class="max-w-[80%] p-4 rounded-2xl text-sm font-medium shadow-sm ${type === 'system' ? 'bg-slate-200 text-slate-700 italic border border-slate-300 text-center w-full max-w-full rounded-2xl my-2' : (isAgent ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-white text-slate-800 rounded-tl-none border border-slate-100')} relative z-30 whitespace-pre-wrap">${type === 'system' ? '<span class="material-symbols-outlined text-[14px] align-middle mr-1">info</span>' : ''}${displayText}${mediaHtml}</div></div>`;
        area.scrollTop = area.scrollHeight;
    },
    ...
    async closeTicket() {
        ...
        if (!tag1 || !tag2 || !notes) { alert("Para encerrar o atendimento, e obrigatorio preencher o Motivo, o Submotivo e as Observacoes do Atendimento."); return; }
        if (!confirm("Tem certeza que deseja encerrar o atendimento atual?")) return;
        try { await agentAPI.closeTicket(agentApp.activeTicketId, tag1, tag2, notes); agentApp.activeTicketId = null; document.getElementById('menu-chat').classList.add('hidden-view'); agentApp.navigate('queue'); Orchestrator.findAndClaimNext(); }
        catch (error) { alert("Erro ao tentar fechar o ticket."); }
    },
    ...
    populateTransferDropdowns() {
        ...
        agentApp.allSubjects.forEach(s => subSelect.innerHTML += `<option value="${s.id}">${s.label}</option>`);
        const agSelect = document.getElementById('transfer-agent');
        agSelect.innerHTML = '<option value="">Para Agente...</option>';
        agentApp.activeAgents.forEach(a => {
            if (a.id !== agentApp.currentUser.id) agSelect.innerHTML += `<option value="${a.id}">${a.full_name}</option>`;
        });
    },

    // ===============================================
    // TRANSFERIR TICKET — grava nota interna de transferencia
    // ===============================================
    async transferTicket() {
        ...
        if (confirm("Voce quer mesmo transferir este chamado para outra pessoa/fila?")) {
            try {
                const fromAgent = agentApp.currentUser.full_name;
                const toAgent = newAg ? (agentApp.activeAgents.find(a => a.id === newAg)?.full_name || 'outro agente') : 'fila';

                // Grava nota interna (nao visivel ao cliente)
                // Requer que agentAPI.sendInternalNote exista — veja api.js
                if (typeof agentAPI.sendInternalNote === 'function') {
                    await agentAPI.sendInternalNote(agentApp.activeTicketId, `Transferido de ${fromAgent} para ${toAgent}`);
                }

                await agentAPI.transferTicket(agentApp.activeTicketId, newSub, newAg, notes);
                agentApp.activeTicketId = null;
                document.getElementById('menu-chat').classList.add('hidden-view');
                agentApp.navigate('queue');
                Orchestrator.findAndClaimNext();
            } catch (e) { alert("Erro na transferencia."); }
        }
    },
    ...
    async viewPastChat(ticketId, protocolNumber) {
        try {
            ...
            modal.classList.remove('hidden-view');
            let html = `<div class="bg-blue-50 border border-blue-100 p-4 rounded-xl mb-6 relative z-20"><h4 class="text-[10px] font-black text-blue-800 uppercase tracking-widest mb-3 flex items-center gap-1"><span class="material-symbols-outlined text-[14px]">summarize</span> Resumo do Atendimento</h4><div class="grid grid-cols-2 gap-4 text-xs mb-3"><div><div class="font-bold text-blue-900 mb-1">Motivo (Tag 1):</div><div class="text-blue-700 bg-white border border-blue-100 px-2 py-1 rounded font-bold inline-block">${ticket.agent_tag1 || 'Nao classificado'}</div></div><div><div class="font-bold text-blue-900 mb-1">Submotivo (Tag 2):</div><div class="text-blue-700 bg-white border border-blue-100 px-2 py-1 rounded font-bold inline-block">${ticket.agent_tag2 || 'Nao classificado'}</div></div></div><div class="text-xs"><div class="font-bold text-blue-900 mb-1">Observacoes do Analista:</div><div class="text-blue-800 italic bg-white/60 border border-blue-100 p-3 rounded-lg">${ticket.agent_notes || 'Nenhuma observacao registrada.'}</div></div></div><div class="h-px bg-slate-200 mb-4"></div>`;
            if (msgs.length === 0) { html += '<div class="text-center text-slate-400 font-bold">Nao existem mensagens registradas neste protocolo.</div>'; }
            else { html += msgs.map(m => agentApp.formatMonitorMsg(m.content, m.sender_type, m.created_at, m.file_url, m.file_name, m.file_type, m.is_internal_note)).join(''); }
            content.innerHTML = html;
            content.scrollTop = content.scrollHeight;
    ...
    async saveOrder() {
        ...
        try { await agentAPI.createOrder({ customer_id: agentApp.currentCustomer.id, product_name: product, quantity: parseInt(qty), amount: parseFloat(amount.replace(',', '.')) }); document.getElementById('order-product').value = ''; document.getElementById('order-amount').value = ''; document.getElementById('order-form').classList.add('hidden-view'); agentApp.loadCustomerOrders(agentApp.currentCustomer.id); }
        catch (e) { alert("Erro ao tentar salvar o pedido no CRM."); }
    },
    ...
    async loadCustomerOrders(customerId) {
        try {
            ...
            if (orders.length === 0) { container.innerHTML = '<div class="text-xs text-slate-400 font-bold">O cliente nao possui nenhum pedido registrado.</div>'; return; }
    ...
    async loadTeam() {
        try {
            ...
            agentApp.allTeamProfiles = team;
            agentApp.allTeamLogs = logs;
            agentApp.renderTeamTable();
    ...
    renderTeamTable() {
        ...
        if (!agentApp.allTeamProfiles) return;
        ...
        tbody.innerHTML = filteredTeam.map(member => {
            ...
            const timeHtml = `<div class="mb-2">${statusBadge}</div><div class="text-[10px] text-slate-500 font-bold space-y-0.5 whitespace-nowrap"><div><span class="text-green-600">Online:</span> ${fmtTime(onlineSecs)}</div><div><span class="text-amber-600">Pausa:</span> ${fmtTime(pausaSecs)}</div><div><span class="text-purple-600">Backoffice:</span> ${fmtTime(backSecs)}</div></div>`;
            const agentTickets = agentApp.activeTickets.filter(t => t.agent_id === member.id && t.status === 'in_progress');
            let chatAg = agentTickets.filter(t => t.channel === 'web' && t.last_sender !== 'agent').length;
            let chatBl = agentTickets.filter(t => t.channel === 'web' && t.last_sender === 'agent').length;
            let emailAg = agentTickets.filter(t => t.channel === 'email' && t.last_sender !== 'agent').length;
            let emailBl = agentTickets.filter(t => t.channel === 'email' && t.last_sender === 'agent').length;
            let ticketsBadge = '';
            if (member.can_web) ticketsBadge += `<div class="mt-2 bg-blue-50 text-blue-700 text-[10px] font-black px-2 py-1 rounded w-max border border-blue-100">CHAT: ${chatAg} ag. voce | ${chatBl} ag. cliente</div>`;
            if (member.can_email) ticketsBadge += `<div class="mt-1 bg-orange-50 text-orange-700 text-[10px] font-black px-2 py-1 rounded w-max border border-orange-100">E-MAIL: ${emailAg} ag. voce | ${emailBl} ag. cliente</div>`;
            let skillsHTML = '';
            ...
            const selStatus = `<select onchange="agentApp.forceAgentStatus('${member.id}', this.value)" class="text-[10px] font-bold bg-slate-50 border border-slate-200 text-slate-600 rounded p-1 outline-none w-full relative z-30 mb-2"><option value="online" ${member.status === 'online' ? 'selected' : ''}>Forcar Online</option><option value="pausa" ${member.status === 'pausa' ? 'selected' : ''}>Forcar Pausa</option><option value="backoffice" ${member.status === 'backoffice' ? 'selected' : ''}>Forcar Backoffice</option><option value="offline" ${member.status === 'offline' ? 'selected' : ''}>Forcar Offline</option></select>`;
            ...
            return `<tr class="relative z-20 hover:bg-slate-50 transition-colors"><td class="p-5 font-black text-slate-900">${member.full_name}<div class="text-[10px] font-bold text-slate-400 mt-1 uppercase">${member.role}</div>${groupInput}${ticketsBadge}</td><td class="p-5 whitespace-nowrap">${timeHtml}</td><td class="p-5 w-[250px]">${skillsHTML}</td><td class="p-5 text-right w-36">${!member.is_approved ? `<button onclick="agentApp.approveMember('${member.id}')" class="bg-blue-600 text-white px-4 py-2 rounded font-bold text-xs mb-2">Aprovar</button>` : selStatus}${chatBtn}</td></tr>`;
        }).join('');
    },
    ...
    async openInternalChat(targetId = null, targetName = null) {
        ...
        content.innerHTML = '<div class="text-center text-slate-400 text-xs font-bold">Carregando o historico...</div>';
        try {
            if (!agentApp.internalChatTarget) throw new Error("ID do usuario alvo nao fornecido.");
            const msgs = await agentAPI.getInternalMessages(agentApp.currentUser.id, agentApp.internalChatTarget);
            content.innerHTML = '';
            msgs.forEach(m => agentApp.renderInternalMsg(m, m.sender_id === agentApp.currentUser.id));
    ...
    async approveMember(id) {
        if (confirm("Deseja realmente aprovar este analista?")) { await agentAPI.approveUser(id, 'analista'); agentApp.loadTeam(); }
    },

    async toggleChannel(agentId, channel, isEnabled) {
        try { await agentAPI.toggleChannel(agentId, channel, isEnabled); if (!isEnabled) { await agentAPI.releaseTicketsByChannel(agentId, channel); } agentApp.loadTeam(); }
        catch (e) { console.error("Erro toggleChannel", e); agentApp.loadTeam(); }
    },

    async toggleSkill(agentId, subjectId, isAdding) {
        try { await agentAPI.toggleAgentSkill(agentId, subjectId, isAdding); agentApp.loadTeam(); }
        catch (e) { console.error("Erro toggleSkill", e); agentApp.loadTeam(); }
    },
    ...
    filterDashboardByAgent(agentId, agentName) {
        agentApp.dashFilterAgent = agentId;
        const badge = document.getElementById('active-filter-badge');
        if (badge) {
            if (agentId) { badge.classList.remove('hidden-view'); document.getElementById('active-filter-name').innerText = agentName; }
            else { badge.classList.add('hidden-view'); }
        }
        agentApp.renderDashboard();
    },

    filterTable() {
        agentApp.tableSearchQuery = document.getElementById('table-search')?.value?.toLowerCase() || '';
        agentApp.renderDashboard();
    }
};
