...
const App = {
    ...
    async init() {
        ...
        document.getElementById('login-form')?.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                console.error("ERRO FATAL DE LOGIN:", error);
    ...
    async translateText(text, targetLang) {
        if (!text || text.trim() === '' || targetLang === 'auto') return text;
                    ...
                    if (el) {
                        if (t.last_sender === 'customer') {
                            ...
                            if (!t.first_reply_at) {
                                ...
                                // Cliente Retornou (Azul)
                                el.style.backgroundColor = '#2563eb'; 
                                el.style.color = '#ffffff';
                            }
                        } else {
                            // Agente (Cinza Esmaecendo)
    ...
    async executeWarning(ticket) {
        try {
            console.log(`[SLA] Enviando Alerta para HZ-${ticket.protocol_number}`);
    ...
    async executeAutoClose(ticket) {
        try {
            ...
            await agentAPI.closeTicket(ticket.id, 'Outros', 'Abandono', 'Encerrado pelo sistema (Falta de interação).');
    ...
    insertMacroVar(variable) {
        const txt = document.getElementById('cfg-macro'); const start = txt.selectionStart; const end = txt.selectionEnd;
    ...
    async toggleClientUpload(isEnabled) { 
        try { await agentAPI.toggleUpload(this.activeTicketId, isEnabled); this.updateUploadToggleUI(isEnabled); } catch(e) { alert("Erro de permissão."); document.getElementById('toggle-upload').checked = !isEnabled; } 
    },
    ...
    async monitorTicket(ticketId, protocolNumber) {
        ...
        try {
            ...
            for (let m of msgs) {
                if(m.sender_type === 'customer') { m.content = await this.translateText(m.content, this.currentLanguage); }
            ...
            this.monitorSub = agentAPI.subscribeToAllMessages(ticketId, async (msgText, senderType, createdAt, fUrl, fName, fType) => { 
                if(senderType === 'customer') msgText = await this.translateText(msgText, this.currentLanguage);
    ...
    async pickTicket(id) {
        try {
            ...
            if (t.status === 'open' || !t.agent_id) {
                ...
                if (myCount >= 10) { alert("O limite máximo simultâneo foi alcançado!"); this.navigate('queue'); return; }
            ...
            // TRADUZ AS MENSAGENS DO CLIENTE
            for (let m of msgs) {
                if (m.sender_type === 'customer') {
                    m.content = await this.translateText(m.content, this.currentLanguage);
                }
            }
            ...
            if (isEmail) {
                ...
                this.messageSub = agentAPI.subscribeToMessages(id, async (msgText, fUrl, fName, fType) => {
                    const translatedText = await this.translateText(msgText, this.currentLanguage);
                    this.renderMsg(translatedText, 'customer', fUrl, fName, fType);
    ...
    renderEmailThread(ticket, msgs) {
        ...
        if (msgs.length > 1) {
            ...
            for (let i = 1; i < msgs.length; i++) {
                const m = msgs[i]; const isAgent = m.sender_type === 'agent' || m.sender_type === 'system'; const dateStr = new Date(m.created_at).toLocaleString('pt-BR'); const senderName = m.sender_type === 'system' ? 'Sistema Automático' : (isAgent ? 'Nossa Equipe' : (ticket.customers?.full_name || 'Cliente')); const bgColor = isAgent ? 'bg-blue-50 border-blue-100' : 'bg-white border-slate-200';
    ...
    async closeTicket() {
        ...
        if (!tag1 || !tag2 || !notes) { alert("Para encerrar o atendimento, é obrigatório preencher o Motivo, o Submotivo e as Observações."); return; }
    ...
    async renderDashboard() {
        ...
        try {
            const startDate = document.getElementById('dash-start').value; const endDate = document.getElementById('dash-end').value; const groupFilter = document.getElementById('dash-filter-group')?.value;
            this.dashboardTickets = await agentAPI.getDashboardTickets(startDate, endDate); this.allProfiles = await agentAPI.getTeamProfiles();
            ...
            if (groupFilter) { const groupAgentsIds = this.allProfiles.filter(p => p.team_group === groupFilter).map(p => p.id); tickets = tickets.filter(t => groupAgentsIds.includes(t.agent_id)); }
            if (this.dashFilterAgent) { tickets = tickets.filter(t => t.agent_id === this.dashFilterAgent); }

            const open = tickets.filter(t => t.status === 'open').length; const inProgress = tickets.filter(t => t.status === 'in_progress').length; const closedTickets = tickets.filter(t => t.status === 'closed'); const closed = closedTickets.length;
            const npsTickets = closedTickets.filter(t => t.rating !== null); const avgNps = npsTickets.length > 0 ? (npsTickets.reduce((acc, t) => acc + t.rating, 0) / npsTickets.length).toFixed(1) : "0.0"; 
            
            let totalWaitMs = 0; let waitCount = 0; let totalHandleMs = 0; let handleCount = 0;
            closedTickets.forEach(t => {
                ...
                const waitMs = assigned - created; if (waitMs >= 0) { totalWaitMs += waitMs; waitCount++; }
                const handleMs = closed - assigned; if (handleMs >= 0) { totalHandleMs += handleMs; handleCount++; }
            });

            const formatTime = (ms) => { if (ms === 0) return "0m"; const m = Math.floor(ms / 60000); if (m < 60) return `${m}m`; const h = Math.floor(m / 60); const remM = m % 60; return `${h}h ${remM}m`; };
            const avgWaitMs = waitCount > 0 ? totalWaitMs / waitCount : 0; const avgHandleMs = handleCount > 0 ? totalHandleMs / handleCount : 0;
            ...
            if (document.getElementById('stat-tme')) document.getElementById('stat-tme').innerText = formatTime(avgWaitMs); 
            if (document.getElementById('stat-tma')) document.getElementById('stat-tma').innerText = formatTime(avgHandleMs); 
            
            this.updateStatusChart(open + inProgress, closed); this.updateAnalystRanking(closedTickets, this.allProfiles); this.renderClosedCasesTable(tickets, this.allProfiles); this.renderChannelStats(closedTickets);
        } catch (error) { console.error("Erro ao carregar Dashboard Executivo:", error); } finally { if (btnFiltro) btnFiltro.innerHTML = origHtml; }
    },

    renderChannelStats(closedTickets) {
        const container = document.getElementById('channel-stats-container'); if (!container) return;
        const webTickets = closedTickets.filter(t => t.channel === 'web'); const emailTickets = closedTickets.filter(t => t.channel === 'email');

        const calcMetrics = (arr) => {
            let tma = 0, tme = 0;
            arr.forEach(t => { const created = new Date(t.created_at); const closed = new Date(t.closed_at); const assigned = t.assigned_at ? new Date(t.assigned_at) : created; tme += (assigned - created); tma += (closed - assigned); });
            return { vol: arr.length, tme: arr.length ? Math.floor((tme/arr.length)/60000) : 0, tma: arr.length ? Math.floor((tma/arr.length)/60000) : 0 }
        };

        const wM = calcMetrics(webTickets); const eM = calcMetrics(emailTickets);
    ...
    updateStatusChart(open, closed) {
        ...
        window.myChart = new Chart(ctx, { type: 'doughnut', data: { labels: ['Em Aberto/Curso', 'Finalizados'], datasets: [{ data: [open, closed], backgroundColor: ['#3b82f6', '#10b981'], borderWidth: 0, hoverOffset: 4 }] }, options: { responsive: true, maintainAspectRatio: false, cutout: '75%', plugins: { legend: { position: 'bottom', labels: { padding: 20, font: { family: 'Manrope', weight: 'bold' } } } } } });
    },
    ...
    updateAnalystRanking(closedTickets, profiles) {
        ...
        const ranking = filteredProfiles.map(p => { 
            const agentTickets = closedTickets.filter(t => t.agent_id === p.id && t.rating !== null); const avg = agentTickets.length > 0 ? (agentTickets.reduce((acc, t) => acc + t.rating, 0) / agentTickets.length).toFixed(1) : 0; const total = closedTickets.filter(t => t.agent_id === p.id).length;
        ...
        container.innerHTML = ranking.map((r, index) => { 
            const badgeColor = index === 0 ? 'bg-amber-100 text-amber-600 border-amber-200' : 'bg-slate-100 text-slate-600 border-slate-200'; const isSelected = this.dashFilterAgent === r.id;
            return `<div onclick="agentApp.filterDashboardByAgent('${r.id}', '${r.name}')" class="cursor-pointer flex items-center justify-between p-4 bg-slate-50 border ${isSelected ? 'border-blue-500 ring-2 ring-blue-200' : 'border-slate-100'} rounded-2xl transition-all hover:bg-blue-50 relative z-20"><div class="flex items-center gap-3"><div class="w-8 h-8 rounded-full flex items-center justify-center font-black text-xs ${badgeColor} border">${index + 1}º</div><div><div class="font-bold text-slate-700">${r.name}</div><div class="text-[9px] text-slate-400 font-bold uppercase">${r.total} Casos Fechados</div></div></div><span class="px-3 py-1 bg-white border rounded-full font-black text-sm ${r.avg >= 9 ? 'text-green-600' : (r.avg > 0 ? 'text-amber-500' : 'text-slate-400')} shadow-sm" title="Nota Média">${r.avg > 0 ? r.avg.toFixed(1) : '-'}</span></div>`; 
        }).join('');
    },

    filterDashboardByAgent(agentId, agentName) {
        if (this.dashFilterAgent === agentId || !agentId) { this.dashFilterAgent = null; document.getElementById('active-filter-badge').classList.add('hidden-view'); } 
        else { this.dashFilterAgent = agentId; document.getElementById('active-filter-badge').classList.remove('hidden-view'); document.getElementById('active-filter-name').innerText = agentName; }
        this.renderDashboard(); 
    },

    filterTable() {
        this.tableSearchQuery = document.getElementById('table-search')?.value.toLowerCase() || "";
    ...
    renderClosedCasesTable(tickets, profiles) {
        ...
        let filteredTickets = closedTickets.filter(t => {
            if (this.dashFilterAgent && t.agent_id !== this.dashFilterAgent) return false;
            if (this.tableSearchQuery) {
                const searchStr = `${t.protocol_number} ${t.customers?.full_name} ${t.agent_tag1} ${t.agent_tag2} ${t.ticket_subjects?.label}`.toLowerCase();
                if (!searchStr.includes(this.tableSearchQuery)) return false;
            }
            return true;
        });
        ...
        container.innerHTML = filteredTickets.map(t => {
            ...
            let tempoStr = hrs > 0 ? `${hrs}h ${mins}m` : `${mins}m`;

            return `<tr class="hover:bg-slate-50 transition-colors"><td class="p-4 font-black text-slate-900 text-xs">HZ-${t.protocol_number}</td><td class="p-4 text-xs"><div class="font-bold text-slate-800 truncate max-w-[150px]">${t.customers?.full_name || 'Cliente'}</div><div class="text-[10px] text-slate-500 truncate max-w-[150px]">${t.ticket_subjects?.label || 'Sem Assunto Inicial'}</div></td><td class="p-4 text-xs"><div class="font-bold text-blue-700 bg-blue-50 px-2 py-0.5 rounded w-max mb-1 text-[10px] truncate max-w-[150px]" title="${t.agent_tag1}">${t.agent_tag1 || 'Não Tagueado'}</div><div class="font-bold text-slate-600 bg-slate-100 px-2 py-0.5 rounded w-max text-[10px] truncate max-w-[150px]" title="${t.agent_tag2}">${t.agent_tag2 || 'Não Tagueado'}</div></td><td class="p-4 font-bold text-slate-700 text-xs">${agentName}</td><td class="p-4 font-bold text-slate-600 text-xs">${tempoStr}</td><td class="p-4 text-center text-xs font-black ${t.rating >= 9 ? 'text-green-600' : (t.rating >= 7 ? 'text-amber-500' : (t.rating ? 'text-red-500' : 'text-slate-300'))}">${t.rating || '-'}</td><td class="p-4 text-right"><button onclick="agentApp.viewPastChat('${t.id}', '${t.protocol_number}')" class="bg-slate-100 text-slate-500 p-2 rounded-lg hover:bg-blue-100 hover:text-blue-600 transition-colors" title="Ver Histórico Completo"><span class="material-symbols-outlined text-[16px]">visibility</span></button></td></tr>`;
        }).join('');
    },

    exportCSV() {
        if (!this.dashboardTickets || this.dashboardTickets.length === 0) { alert("Não há dados para exportar no período selecionado."); return; }
        ...
        this.dashboardTickets.filter(t => t.status === 'closed').forEach(t => {
            ...
            const created = new Date(t.created_at); const closed = new Date(t.closed_at); const assigned = t.assigned_at ? new Date(t.assigned_at) : null;
            const tmeMins = assigned ? Math.floor((assigned - created) / 60000) : 0; const tmaMins = assigned ? Math.floor((closed - assigned) / 60000) : 0;
            const safeNotes = t.agent_notes ? t.agent_notes.replace(/"/g, '""').replace(/\n/g, ' ') : '';
            
            const row = [ `HZ-${t.protocol_number}`, t.channel, created.toLocaleString('pt-BR'), assigned ? assigned.toLocaleString('pt-BR') : '', closed.toLocaleString('pt-BR'), tmeMins, tmaMins, t.customers?.full_name || '', t.customers?.email || '', t.ticket_subjects?.label || '', t.agent_tag1 || '', t.agent_tag2 || '', `"${safeNotes}"`, agentName, t.rating || '' ].join(';');
            csvContent += row + "\n";
        });

        const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.setAttribute("href", url); link.setAttribute("download", `horizon_relatorio_${new Date().toISOString().split('T')[0]}.csv`); document.body.appendChild(link); link.click(); document.body.removeChild(link);
    },
    ...
    async openInternalChat(targetId = null, targetName = null) {
        if (targetId) { this.internalChatTarget = targetId; document.getElementById('internal-chat-target-name').innerText = targetName; }
        ...
        try {
            ...
            content.innerHTML = ''; msgs.forEach(m => this.renderInternalMsg(m, m.sender_id === this.currentUser.id));
    ...
    async approveMember(id) { if(confirm("Deseja realmente aprovar este analista?")) { await agentAPI.approveUser(id, 'analista'); this.loadTeam(); } },
    async toggleChannel(agentId, channel, isEnabled) { try { await agentAPI.toggleChannel(agentId, channel, isEnabled); if (!isEnabled) { await agentAPI.releaseTicketsByChannel(agentId, channel); } this.loadTeam(); } catch (e) { console.error("Erro toggleChannel", e); this.loadTeam(); } },
    async toggleSkill(agentId, subjectId, isAdding) { try { await agentAPI.toggleAgentSkill(agentId, subjectId, isAdding); this.loadTeam(); } catch (e) { console.error("Erro toggleSkill", e); this.loadTeam(); } }
};
