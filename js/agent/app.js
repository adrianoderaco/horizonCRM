...
const App = {
    ...
    async init() {
        ...
        document.getElementById('login-form')?.addEventListener('submit', async (e) => {
            ...
            btn.innerHTML = `<span class="material-symbols-outlined animate-spin notranslate">refresh</span> Processando...`;
            
            const email = document.getElementById('login-email').value; 
            const pass = document.getElementById('login-pass').value;

            if (agentApp.isRegisterMode) {
                try { await agentAPI.register(document.getElementById('reg-name').value, email, pass); alert("Aguarde aprovação."); agentApp.toggleAuthMode(); } 
            ...
            try {
                ...
                agentAPI.subscribeToSettings((newCfg) => {
                    agentApp.systemSettings = newCfg; Orchestrator.updateSettings(newCfg);
                ...
                agentAPI.subscribeToProfiles(async () => { if (!document.getElementById('sec-team').classList.contains('hidden-view')) await agentApp.loadTeam(); });
                ...
                agentAPI.subscribeToMyProfile(agentApp.currentUser.id, async (newProfile) => {
                    if (newProfile.status !== agentApp.currentUser.status) {
                        ...
                        if (newProfile.status !== 'online') {
                            ...
                            agentApp.navigate('queue'); alert("Atenção: Seu status foi alterado para " + newProfile.status.toUpperCase() + " pelo Gestor.");
            ...
            } catch (error) { alert(`Erro: ${error.message}`); btn.innerHTML = orig; }
        });
        ...
        document.getElementById('agent-email-form')?.addEventListener('submit', async (e) => {
            ...
            if(!text || !agentApp.activeTicketId) return;
            if(!confirm("Prosseguir com a resposta ao cliente? O ticket sairá da tela.")) return;
            const btn = document.getElementById('btn-send-email'); const origHTML = btn.innerHTML;
            btn.innerHTML = `<span class="material-symbols-outlined animate-spin notranslate">refresh</span> Enviando...`; btn.disabled = true;
            try {
            ...
            } catch(err) { alert("Erro."); } finally { btn.innerHTML = origHTML; btn.disabled = false; }
        });
        ...
        document.getElementById('internal-chat-form')?.addEventListener('submit', async (e) => {
            ...
            try { await agentAPI.sendInternalMessage(agentApp.currentUser.id, agentApp.internalChatTarget, text); agentApp.renderInternalMsg({ content: text }, true); input.value = ''; } catch (err) { alert("Erro."); }
        });
    },
    ...
    async toggleGlobalOrchestrator(isActive) {
        try { await agentAPI.updateSystemSettings({ is_orchestrator_active: isActive }); } catch (e) { document.getElementById('toggle-routing').checked = !isActive; alert("Erro."); }
    },
    ...
    setupAgentAttachment(btnId, inputId, isEmail = false) {
        ...
        if (btn && input) {
            ...
            input.addEventListener('change', async (e) => {
                ...
                btnSend.innerHTML = `<span class="material-symbols-outlined animate-spin notranslate">refresh</span>`; btnSend.disabled = true;
                try {
    ...
    async forceAgentStatus(agentId, newStatus) {
        ...
        try { await agentAPI.changeStatus(agentId, newStatus); if (newStatus !== 'online') await agentAPI.releaseMyTickets(agentId); await agentApp.loadTeam(); } catch(e) { alert("Erro."); }
    },

    async updateAgentLimits(agentId, maxChats, maxEmails) {
        try { let c = maxChats === "" ? null : maxChats; let e = maxEmails === "" ? null : maxEmails; await agentAPI.updateAgentLimits(agentId, c, e); } catch (e) { alert("Erro."); }
    },
    
    async updateAgentGroup(agentId, groupName) {
        try { await agentAPI.updateAgentGroup(agentId, groupName); } catch (e) { alert("Erro."); }
    },

    applyWatermark(name) {
        const wm = document.createElement('div'); wm.className = 'fixed inset-0 pointer-events-none flex flex-wrap overflow-hidden justify-center items-center select-none notranslate'; wm.style.zIndex = '99999'; 
    ...
    async logout() {
        if(confirm("Deseja sair? Seus atendimentos em andamento voltarão para a fila!")) {
            try { await agentAPI.releaseMyTickets(agentApp.currentUser.id); await agentAPI.changeStatus(agentApp.currentUser.id, 'offline'); await supabase.auth.signOut(); location.reload(); } catch(e) { alert("Erro."); }
        }
    },
    ...
    toggleAuthMode() {
        ...
        document.getElementById('btn-login').innerHTML = agentApp.isRegisterMode ? 'Criar Conta' : 'Entrar <span class="material-symbols-outlined notranslate">login</span>';
    ...
    async addTag1() {
        ...
        try { await agentAPI.createSubject(val); input.value = ''; agentApp.allSubjects = await agentAPI.getAllSubjects(); agentApp.renderSettingsTags(); } catch(e) { alert("Erro ao criar."); }
    },

    async addTag2(subjectId) {
        const input = document.getElementById(`new-tag2-${subjectId}`); const val = input.value.trim(); if(!val) return;
        try { await agentAPI.createSubsubject(subjectId, val); input.value = ''; agentApp.allSubsubjects = await agentAPI.getAllSubsubjects(); agentApp.renderSettingsTags(); } catch(e) { alert("Erro ao criar."); }
    },
    ...
    async toggleSubsubject(id, isActive) {
        try { await agentAPI.toggleSubsubject(id, isActive); agentApp.allSubsubjects = await agentAPI.getAllSubsubjects(); agentApp.renderSettingsTags(); } catch(e) { alert("Erro ao excluir."); }
    },
    ...
    async saveSettings() {
        const btn = document.getElementById('btn-save-cfg'); const origHtml = btn.innerHTML; btn.innerHTML = `<span class="material-symbols-outlined animate-spin notranslate">refresh</span> Salvando...`;
        try {
    ...
    async toggleClientUpload(isEnabled) { 
        try { await agentAPI.toggleUpload(agentApp.activeTicketId, isEnabled); agentApp.updateUploadToggleUI(isEnabled); } catch(e) { alert("Erro."); document.getElementById('toggle-upload').checked = !isEnabled; } 
    },
            ...
            if (isGestor) {
                ...
                tbody.innerHTML = tickets.map(t => {
                    ...
                    let chBadge = t.channel === 'email' 
                        ? `<span class="text-[10px] bg-orange-100 text-orange-700 border border-orange-200 px-2 py-0.5 rounded ml-2 font-bold flex items-center w-max gap-1"><span class="material-symbols-outlined notranslate text-[10px]">mail</span> E-MAIL</span>` 
                        : `<span class="text-[10px] bg-slate-100 text-slate-600 border border-slate-200 px-2 py-0.5 rounded ml-2 font-bold flex items-center w-max gap-1"><span class="material-symbols-outlined notranslate text-[10px]">forum</span> CHAT</span>`;
                    ...
                    <select onchange="agentApp.reassignTicket('${t.id}', this.value)" class="mt-1 text-[10px] font-bold bg-slate-50 border border-slate-200 text-slate-600 rounded p-1 outline-none w-full max-w-[150px] relative z-30">
                        ...
                        ${agentApp.activeAgents.map(a => `<option value="${a.id}" ${a.id === t.agent_id ? 'selected' : ''}>${a.full_name}</option>`).join('')}
                    </select>`;
                    
                    let actionBtn = inProg && !isMine 
                        ? `<button onclick="agentApp.monitorTicket('${t.id}', '${t.protocol_number}')" class="bg-blue-100 text-blue-700 px-4 py-2.5 rounded-xl text-xs font-black hover:bg-blue-200 transition-all flex items-center gap-1 justify-center w-full relative z-30"><span class="material-symbols-outlined notranslate text-[16px]">visibility</span> Monitorar</button>` 
                ...
                if (agentApp.currentUser.status !== 'online') { msgEl.innerHTML = `Você está <strong class="uppercase">${agentApp.currentUser.status}</strong>.<br>Mude para ONLINE para receber chamados da fila.`; } 
                else { msgEl.innerHTML = `O Orquestrador enviará novos chamados automaticamente.<br>Fique atento às bolhas no cabeçalho da sua tela.`; }
            }
    ...
    async monitorTicket(ticketId, protocolNumber) {
        ...
        try {
            ...
            <div class="bg-blue-50 border border-blue-100 p-4 rounded-xl mb-6 relative z-20">
                <h4 class="text-[10px] font-black text-blue-800 uppercase tracking-widest mb-3 flex items-center gap-1"><span class="material-symbols-outlined notranslate text-[14px]">summarize</span> Resumo do Atendimento</h4>
                <div class="grid grid-cols-2 gap-4 text-xs mb-3"><div><div class="font-bold text-blue-900 mb-1">Motivo (Tag 1):</div><div class="text-blue-700 bg-white border border-blue-100 px-2 py-1 rounded font-bold inline-block">${ticket.agent_tag1 || 'Não classificado'}</div></div><div><div class="font-bold text-blue-900 mb-1">Submotivo (Tag 2):</div><div class="text-blue-700 bg-white border border-blue-100 px-2 py-1 rounded font-bold inline-block">${ticket.agent_tag2 || 'Não classificado'}</div></div></div>
            ...
            </div><div class="h-px bg-slate-200 mb-4"></div>`;
            
            msgs.forEach(m => html += agentApp.formatMonitorMsg(m.content, m.sender_type, m.created_at, m.file_url, m.file_name, m.file_type));
        ...
        } catch(e) { content.innerHTML = '<div class="text-center text-red-400 font-bold mt-4">Erro.</div>'; }
    },
    ...
    async pickTicket(id) {
        try {
            ...
            if (t.status === 'open' || !t.agent_id) {
                ...
                if (myCount >= 10) { alert("Limite de atendimentos simultâneos alcançado!"); agentApp.navigate('queue'); return; }
            ...
            document.getElementById('chat-header-channel').innerHTML = isEmail ? `<span class="material-symbols-outlined notranslate text-[14px]">mail</span> E-MAIL` : `<span class="material-symbols-outlined notranslate text-[14px]">forum</span> CHAT WEB`; 
            ...
            if (isEmail) {
                ...
                agentApp.messageSub = agentAPI.subscribeToMessages(id, (msgText, fUrl, fName, fType) => {
                    agentApp.playAlert('message'); // Som para nova msg
    ...
    async transferTicket() {
        ...
        if (confirm("Você quer mesmo transferir este chamado?")) { try { await agentAPI.transferTicket(agentApp.activeTicketId, newSub, newAg, notes); agentApp.activeTicketId = null; document.getElementById('menu-chat').classList.add('hidden-view'); agentApp.navigate('queue'); Orchestrator.findAndClaimNext(); } catch (e) { alert("Erro na transferência."); } }
    },
    ...
    async viewPastChat(ticketId, protocolNumber) {
        try {
            ...
            let html = `<div class="bg-blue-50 border border-blue-100 p-4 rounded-xl mb-6 relative z-20"><h4 class="text-[10px] font-black text-blue-800 uppercase tracking-widest mb-3 flex items-center gap-1"><span class="material-symbols-outlined notranslate text-[14px]">summarize</span> Resumo do Atendimento</h4><div class="grid grid-cols-2 gap-4 text-xs mb-3"><div><div class="font-bold text-blue-900 mb-1">Motivo (Tag 1):</div><div class="text-blue-700 bg-white border border-blue-100 px-2 py-1 rounded font-bold inline-block">${ticket.agent_tag1 || 'Não classificado'}</div></div><div><div class="font-bold text-blue-900 mb-1">Submotivo (Tag 2):</div><div class="text-blue-700 bg-white border border-blue-100 px-2 py-1 rounded font-bold inline-block">${ticket.agent_tag2 || 'Não classificado'}</div></div></div><div class="text-xs"><div class="font-bold text-blue-900 mb-1">Observações do Analista:</div><div class="text-blue-800 italic bg-white/60 border border-blue-100 p-3 rounded-lg">${ticket.agent_notes || 'Nenhuma observação registrada.'}</div></div></div><div class="h-px bg-slate-200 mb-4"></div>`;
            if(msgs.length === 0) { html += '<div class="text-center text-slate-400 font-bold">Não existem mensagens registradas neste protocolo.</div>'; } else { html += msgs.map(m => agentApp.formatMonitorMsg(m.content, m.sender_type, m.created_at, m.file_url, m.file_name, m.file_type)).join(''); }
            content.innerHTML = html;
        } catch (e) { alert("Erro ao abrir a conversa."); }
    },
    ...
    async loadCustomerOrders(customerId) {
        try {
            ...
            if(orders.length === 0) { container.innerHTML = '<div class="text-xs text-slate-400 font-bold">O cliente não possui pedidos.</div>'; return; }
    ...
    renderTeamTable() {
        ...
        tbody.innerHTML = filteredTeam.map(member => {
            ...
            const chatBtn = member.id !== agentApp.currentUser.id ? `<button onclick="agentApp.openInternalChat('${member.id}', '${member.full_name}')" class="bg-slate-700 text-white px-3 py-1.5 rounded-lg text-[10px] font-black hover:bg-slate-800 transition-all flex items-center justify-center w-full gap-1"><span class="material-symbols-outlined notranslate text-[12px]">forum</span> Falar c/ Agente</button>` : '';
    ...
    async openInternalChat(targetId = null, targetName = null) {
        ...
        } catch(e) { content.innerHTML = `<div class="text-center text-red-400 text-xs font-bold">Falha ao abrir a conversa.</div>`; }
    },
