...
const App = {
    ...
    async init() {
        ...
        document.getElementById('internal-chat-form')?.addEventListener('submit', async (e) => {
            ...
            try { await agentAPI.sendInternalMessage(agentApp.currentUser.id, agentApp.internalChatTarget, text); agentApp.renderInternalMsg({ content: text }, true); input.value = ''; } catch (err) { alert("Erro ao enviar mensagem interna."); }
        });
    },

    // ===============================================
    // LÓGICA DO IDIOMA GLOBAL (GOOGLE TRANSLATE)
    // ===============================================
    changeLanguage(lang) {
        const changeGoogleLanguage = (targetLang) => {
        ...
        setTimeout(() => changeGoogleLanguage(lang), 200);
    },
    ...
    async toggleGlobalOrchestrator(isActive) {
        try { await agentAPI.updateSystemSettings({ is_orchestrator_active: isActive }); } catch (e) { document.getElementById('toggle-routing').checked = !isActive; alert("Erro ao alterar Orquestrador Global."); }
    },
    ...
    // ===============================================
    // TIMERS E SLA (Lógica Matemática Consertada e Tagueamento Automático)
    // ===============================================
    startLiveTimers() {
    ...
    async executeAutoClose(ticket) {
        try {
            ...
            await agentApp.loadQueue();
        } catch(e) { console.error("Falha na macro de encerramento SLA:", e); } finally { agentApp.closingTickets.delete(ticket.id); }
    },
    ...
    async toggleClientUpload(isEnabled) { 
        try { await agentAPI.toggleUpload(agentApp.activeTicketId, isEnabled); agentApp.updateUploadToggleUI(isEnabled); } catch(e) { alert("Erro de permissão ao ativar anexo."); document.getElementById('toggle-upload').checked = !isEnabled; } 
    },
    ...
    async pickTicket(id) {
        try {
            ...
            if (t.status === 'open' || !t.agent_id) {
                ...
                if (myCount >= 10) { alert("O limite máximo de atendimentos simultâneos foi alcançado!"); agentApp.navigate('queue'); return; }
            ...
            if (isEmail) {
                ...
                agentApp.messageSub = agentAPI.subscribeToMessages(id, (msgText, fUrl, fName, fType) => {
                    agentApp.playAlert('message'); // Som para nova mensagem do cliente no ticket ativo
    ...
    async closeTicket() {
        ...
        try { await agentAPI.closeTicket(agentApp.activeTicketId, tag1, tag2, notes); agentApp.activeTicketId = null; document.getElementById('menu-chat').classList.add('hidden-view'); agentApp.navigate('queue'); Orchestrator.findAndClaimNext(); } catch (error) { alert("Erro ao tentar fechar o ticket."); }
    },
    ...
    async viewPastChat(ticketId, protocolNumber) {
        try {
            ...
            <div class="bg-blue-50 border border-blue-100 p-4 rounded-xl mb-6 relative z-20"><h4 class="text-[10px] font-black text-blue-800 uppercase tracking-widest mb-3 flex items-center gap-1"><span class="material-symbols-outlined text-[14px]">summarize</span> Resumo do Atendimento</h4><div class="grid grid-cols-2 gap-4 text-xs mb-3"><div><div class="font-bold text-blue-900 mb-1">Motivo (Tag 1):</div><div class="text-blue-700 bg-white border border-blue-100 px-2 py-1 rounded font-bold inline-block">${ticket.agent_tag1 || 'Não classificado'}</div></div><div><div class="font-bold text-blue-900 mb-1">Submotivo (Tag 2):</div><div class="text-blue-700 bg-white border border-blue-100 px-2 py-1 rounded font-bold inline-block">${ticket.agent_tag2 || 'Não classificado'}</div></div></div><div class="text-xs"><div class="font-bold text-blue-900 mb-1">Observações do Analista:</div><div class="text-blue-800 italic bg-white/60 border border-blue-100 p-3 rounded-lg">${ticket.agent_notes || 'Nenhuma observação registrada.'}</div></div></div><div class="h-px bg-slate-200 mb-4"></div>
            `;
            if(msgs.length === 0) { html += '<div class="text-center text-slate-400 font-bold">Não existem mensagens registradas neste protocolo.</div>'; } 
            else { html += msgs.map(m => agentApp.formatMonitorMsg(m.content, m.sender_type, m.created_at, m.file_url, m.file_name, m.file_type)).join(''); }
            content.innerHTML = html; content.scrollTop = content.scrollHeight;
    ...
    async toggleChannel(agentId, channel, isEnabled) { 
        try { await agentAPI.toggleChannel(agentId, channel, isEnabled); if (!isEnabled) { await agentAPI.releaseTicketsByChannel(agentId, channel); } agentApp.loadTeam(); } catch (e) { console.error("Erro toggleChannel", e); agentApp.loadTeam(); } 
    },
    
    async toggleSkill(agentId, subjectId, isAdding) { 
        try { await agentAPI.toggleAgentSkill(agentId, subjectId, isAdding); agentApp.loadTeam(); } catch (e) { console.error("Erro toggleSkill", e); agentApp.loadTeam(); } 
    }
};
