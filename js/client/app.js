...
const App = {
    selectedSubject: null, channel: null, ticketId: null,

    async init() {
        window.clientApp = this; await this.loadSubjects();
        ...
        document.getElementById('register-form').addEventListener('submit', async (e) => {
            ...
            const name = document.getElementById('cust-name').value; const email = document.getElementById('cust-email').value; const phone = document.getElementById('cust-phone').value; const initialMessage = document.getElementById('cust-message').value;
            ...
            try {
                ...
                if (this.channel === 'email') { this.navigate('success-email'); document.getElementById('success-protocol').innerText = `Protocolo HZ-${ticket.protocol_number}`; return; }
                
                this.navigate('chat'); document.getElementById('header-desc').innerText = `Protocolo HZ-${ticket.protocol_number}`;
                ...
                clientAPI.subscribeToTicket(this.ticketId, (t) => {
                    ...
                    if (btnAttach) { if (t.is_upload_enabled) btnAttach.classList.remove('hidden-view'); else btnAttach.classList.add('hidden-view'); }
                });

                clientAPI.subscribeToMessages(this.ticketId, (msg, fUrl, fName, fType) => { this.renderMsg(msg, 'agent', fUrl, fName, fType); });
        ...
        if (btnAttach && fileInput) {
            ...
            fileInput.addEventListener('change', async (e) => {
                ...
                document.getElementById('upload-progress').classList.remove('hidden-view'); document.getElementById('btn-send').disabled = true;
                try {
    ...
    navigate(target) { ['subjects', 'channel', 'register', 'chat', 'nps', 'success-email'].forEach(s => { const el = document.getElementById(`sec-${s}`); if (el) el.classList.add('hidden-view'); }); const targetEl = document.getElementById(`sec-${target}`); if (targetEl) targetEl.classList.remove('hidden-view'); },
    async loadSubjects() {
        const container = document.getElementById('subject-list'); if (!container) return;
        try {
            const subjects = await clientAPI.getActiveSubjects(); if (!subjects || subjects.length === 0) return;
    ...
    renderMsg(text, type, fileUrl = null, fileName = null, fileType = null) {
        ...
        else { const isMe = type === 'customer'; area.innerHTML += `<div class="flex ${isMe ? 'justify-end' : 'justify-start'} w-full"><div class="max-w-[85%] p-4 rounded-2xl text-sm font-medium shadow-sm ${isMe ? 'bg-slate-900 text-white rounded-tr-none' : 'bg-white border border-slate-200 text-slate-800 rounded-tl-none'}">${text}${mediaHtml}</div></div>`; }
        area.scrollTop = area.scrollHeight;
    },

    renderNPSButtons() {
        const container = document.getElementById('nps-buttons'); if (!container) return;
        let html = '';
        for(let i=1; i<=10; i++) {
            ...
            // flex-1 min-w-0 garante que eles encolham uniformemente
