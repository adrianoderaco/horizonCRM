...
export const clientAPI = {
    ...
    async createTicket(customerId, subjectId, channel) {
        const { data, error } = await supabase.from('tickets').insert([{ customer_id: customerId, subject_id: subjectId, channel: channel }]).select().single();
        if (error) throw error;
        return data;
    },
    ...
    subscribeToTicket(ticketId, onUpdate) {
        return supabase.channel(`client-status-${ticketId}`).on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'tickets', filter: `id=eq.${ticketId}` }, payload => { onUpdate(payload.new); }).subscribe();
    },
    subscribeToMessages(ticketId, onNewMessage) {
        return supabase.channel(`client-ticket-${ticketId}`).on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `ticket_id=eq.${ticketId}` }, payload => {
            if (payload.new.sender_type === 'agent') { onNewMessage(payload.new.content, payload.new.file_url, payload.new.file_name, payload.new.file_type); }
        }).subscribe();
    },
